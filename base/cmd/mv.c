 ...
 *	  mv file ... directory
 * SLOW selects the simpler copy loop: it tests the whole buffer for a hole in
 * one pass instead of testing each block of it.
 */
...
/*
 * The copy buffer, for a move between filesystems.  BUFSIZ is the filesystem
 * block size, which is the unit the copy works in whatever the buffer holds:
 * a hole is a block of zeros, so the scan is per block, and every write is one
 * block.
 */
char	buf[BUFSIZ];
