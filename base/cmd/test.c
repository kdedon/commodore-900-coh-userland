...
/*
 * PROTO, ARGS, VOID and USE_PROTO come from <sys/compat.h> on a 4.x system.
 * This system has no such header; these are the definitions 4.x's own
 * <common/_ccompat.h> selects for a K&R COHERENT cc -- prototypes dropped from
 * declarations, ARGS an identity macro, and `char' standing in for `void'
 * where a pointer to it is wanted.
 */
#define	PROTO(p)	()
#define	ARGS(x)		x
#define	VOID		char
...
#endif
{
	...
	/*
	 * Integer operands are decimal, and a partly-numeric operand is an
	 * error rather than the value of its numeric prefix: `08' is eight,
	 * `0x10' is not sixteen and not zero either.  strtol leaves cp at the
	 * start of the string when it converted nothing, which is how the
	 * empty argument still reads as zero.
	 */
	*numberp = strtol (string, & cp, 10);
	...
	if (argc >= 3) {
		...
		* matchedp = 3;
		...
		if ((cmp = is_string_binop (argv [argc - 2])) != NULL) {
			result = (* cmp) ((long) strcmp (argv [argc - 3],
							 argv [argc - 1]),
					  0L);
			goto negate;
		}
...
#define	WRITECONST(s)	write (2, s, sizeof (s) - 1)
