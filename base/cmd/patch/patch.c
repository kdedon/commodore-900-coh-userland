...
"%seversed (or previously applied) patch detected!  %s -R? [y] ",
		...
		failed++;
		if (verbose)
		    say3("Hunk #%ld ignored at %ld.\n", (long)hunk, newwhere);
	    }
	    else if (where == Nulline) {
		abort_hunk();
		failed++;
		if (verbose)
		    say3("Hunk #%ld failed at %ld.\n", (long)hunk, newwhere);
	    }
	    else {
		apply_hunk(where);
		if (verbose) {
		    say3("Hunk #%ld succeeded at %ld", (long)hunk, newwhere);
		    if (fuzz)
	...
	if (failed) {
	    ...
	    if (skip_rest_of_patch) {
		say4("%ld out of %ld hunks ignored--saving rejects to %s\n",
		    (long)failed, (long)hunk, rejname);
	    }
	    else {
		say4("%ld out of %ld hunks failed--saving rejects to %s\n",
		    (long)failed, (long)hunk, rejname);
	    }
...
#define IN_ELSE 3
	...
	if (pch_char(old) == '-') {
		...
		pch_hunk_beg() + new);
#ifdef DEBUGGING
	    say3("oldchar = %ld, newchar = %ld\n",
		(long)pch_char(old), (long)pch_char(new));
#endif
	    my_exit(1);
	}
	else if (pch_char(new) == '!') {
...
LINENUM fuzz;
{
	...
	/* An input line is a slice of the whole-file image and carries no
	 * terminator, so exactly the pattern line's length is compared.
	 */
	else if (memcmp(ifetch(iline, (offset >= 0)),
		   pfetch(pline),
		   (unsigned)pch_line_len(pline) ))
	    return FALSE;
    }
    return TRUE;
}
