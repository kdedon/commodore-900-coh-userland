...
char *filename;
{
    ...
    if (i_size + 2 > MAXPLANAFILE)
	return FALSE;		/* the whole file will not fit in one object */
#ifdef lint
    i_womp = Nullch;
#else
...
#ifndef lint
    /* read() counts a chunk in an int, so a file larger than one is read in
     * pieces.
     */
    {
	long left = i_size;
	...
	while (left > 0) {
	    chunk = (left > 16384L ? 16384 : (int)left);
	    got = read(ifd, p, chunk);
	    if (got != chunk) {
		Close(ifd);
		free(i_womp);
		i_womp = Nullch;
    ...
    if (iline + 2 > MAXPLANALINES) {	/* the index will not fit in one object */
	free(i_womp);
	i_womp = Nullch;
	return FALSE;
    }
...
char *string;
{
    ...
    if (nstreq(string,revision,(unsigned)patlen) && isspace(string[patlen]))
	return TRUE;
    for (s = string; *s; s++) {
	if (isspace(*s) && nstreq(s+1, revision, (unsigned)patlen) && 
		isspace(s[patlen+1] )) {
	    return TRUE;
	}
    }
