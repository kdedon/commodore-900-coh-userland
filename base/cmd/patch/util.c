...
/* True if the first n bytes of two strings are equal, a NUL in both ending
 * the comparison early.  Used where one side is the whole-file image, which
 * is far longer than the n bytes of interest.
 */
...
#ifdef DEBUGGING
    if (debug & 128)
	say4("fetchname %s %ld %ld\n", name, (long)strip_leading,
	  (long)assume_exists);
#endif
