...
/*
 * This system's terminal interface is the V7 one: TIOCGETP/TIOCSETP over a
 * struct sgttyb, which is the distribution's SGTTY branch.  <sgtty.h> is
 * included directly because there is no <sys/ioctl.h> here.
 */
#include <sgtty.h>
#define SGTTY
#include "screen.h"
...
/*
 * libterm's own globals, which it leaves for its caller to fill: tputs()
 * defines and reads ospeed, and tgoto() reads UP and BC when it has to reach
 * a column by backing up.
 */
extern short ospeed;
char *UP;
char *BC;
...

{
    ...
    /* cursor up and backspace, for tgoto's %B and column back-up */
    UP = tgetstr("up", &bufptr);
    BC = tgetstr("bc", &bufptr);
...

{
	...
	/* tputs() computes pad counts from the line's output speed */
	ospeed = old_settings.sg_ospeed;
