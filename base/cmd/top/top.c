...
/*
 * The buffer stdio will use for stdout.  setbuf() here takes no length: the
 * length is the compiled-in BUFSIZ, so the buffer must be exactly that.
 */
...

{
    ...
    char numbuf[16];		/* numeric arguments to new_message */
...
#define CMD_user	14
...
#else
    setbuffer(stdout, stdoutbuf, BUFSIZ);
#endif
	...
	while ((i = getopt(ac, av, "SIbinqus:d:U:")) != EOF)
	{
	    switch(i)
	    {
		...
		fprintf(stderr, "\
Top version %s\n\
Usage: %s [-ISbinqu] [-d x] [-s x] [-U username] [number]\n",
    ...
    /*
     * Take the terminal, then the signals that have to put it back.  There
     * is no sigblock() here to hold them off in between; an interrupt landing
     * inside init_screen() leaves the terminal in cbreak with echo off, which
     * is what `stty sane' is for.  SIGTSTP does not exist on this system.
     */
    init_screen();
	...
	processes =
		get_process_info(&system_info, &ps, proc_compare);
		...
		if (select(32, &readfds, (fd_set *)NULL, (fd_set *)NULL, &timeout) > 0)
		{
			...
			else switch(change)
			{
				...
				if (newval > -1)
				{
				    if (newval > max_topn)
				    {
					(void) sprintf(numbuf, "%d", max_topn);
					new_message(MT_standout | MT_delayed,
					  " This terminal can only display %s processes.",
					  numbuf);
					putchar('\r');
				    }
