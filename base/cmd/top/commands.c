...
r       - renice a process (this kernel has no setpriority(2))\n\
 ...
 *
 *  This kernel has no setpriority(2), and its nice(2) is the V7 one: an
 *  increment applied to the calling process only.  There is no call that
 *  changes another process's nice value, so the command reports that rather
 *  than doing something else under the same key.  The key and its help line
 *  stay, so the interface does not change shape if the call arrives.
 */
...
{
    return(" renice: this kernel has no setpriority(2)");
}
