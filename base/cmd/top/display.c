...
/*
 * Whether the process-area header is drawn.  ERASE means it was drawn and is
 * to be removed on the next update.
 */
#define	HDR_OFF		0
#define	HDR_ON		1
#define	HDR_ERASE	2

static int header_status = HDR_ON;
...

{
    ...
    if (lines > -1)
    {
	/* save pointers and allocate space for procstate and cpustate names */
...
/*
 * Format one load average into "buf" and return it.  A load average is
 * hundredths in a long; LOAD_NONE means the kernel keeps none, and prints as
 * dashes in the same five columns a number would have taken, so a machine
 * that gains one later needs no change to the layout above or below it.
 */
static char *loadfmt(la, buf)
...

{
    ...
    {
	(void) sprintf(buf, "%2ld.%02ld", la / LOAD_SCALE, la % LOAD_SCALE);
    }
    return(buf);
}
...

{
	...
	if (*thisname != '\0')
	{
	    ...
	    if (value >= 1000)
	    {
		printf("%s 100%% %s", i++ == 0 ? "" : ", ", thisname);
	    }
	    else
	    {
		printf("%s%2d.%d%% %s", i++ == 0 ? "" : ", ",
		       value / 10, value % 10, thisname);
	    }
	}
    }
}
...

{
    ...
    if (header_status == HDR_ON)
    {
	putchar('\n');
	fputs(text, stdout);
	lastline++;
    }
    else if (header_status == HDR_ERASE)
    {
	header_status = HDR_OFF;
    }
}
...

{
    if (t)
    {
	header_status = HDR_ON;
    }
    else if (header_status == HDR_ON)
    {
	header_status = HDR_ERASE;
    }
}

/*
 * Post a message in the message area.
 *
 * Every variable argument is a string, and callers with a number to show
 * format it themselves: a call that sometimes passes a pointer and sometimes
 * an int pushes two bytes where four are read.
 */
/*VARARGS2*/
...

{
	...
	if (num > 0)
	{
	    /* is this number in kilobytes and it is larger than 9999? */
