...
/*
 *  ltoa(val) - itoa for a long.  Sizes in K and counts of clock ticks do not
 *	fit an int on this machine.
 */
...

{
    ...
    {
	/*
	 * Hours and tenths.  The distribution prints this with %5.1f; the
	 * quantity is a whole number of tenths of an hour, so it is divided
	 * out and printed as one, which costs no accuracy and keeps the
	 * program clear of double.
	 */
	tenths = (seconds * 10l + 1800l) / 3600l;
	sprintf(result, "%3ld.%1ldH", tenths / 10l, tenths % 10l);
    ...
    {
	/*
	 * The standard method produces MMM:SS.  The conversions are %ld
	 * because the arguments are long: int is 16 bits here, so a %d would
	 * read two bytes of the four that were pushed.
	 */
