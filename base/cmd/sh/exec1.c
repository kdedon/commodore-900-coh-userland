...
register NODE *np;
{
	...
	for ( ; np; np = np->n_next) {
		...
		case NIORS:
			...
			for (s1 = np->n_strp, sp = strt; s1 < s; )
				*sp++ = *s1++;
			/*
			 * The word can expand to NO words -- `> $EMPTY', an
			 * unmatched glob -- and nargv[nargs] is then the
			 * vector's terminator, not a word.  Leave the
			 * redirection naming nothing and let the open report
			 * it; taking an argument back off the vector here
			 * would take one that was never put on.
			 */
			if (nargv[nargs] == NULL)
				*sp = '\0';
			else {
				strcpy(sp, nargv[nargs]);
				--nargc;
				for (app = nargv+nargs; *app; app++) {
					*app = *(app + 1);   /* shift down */
					if (xflag && *app != NULL)
	...
	/*
	 * Restricted shell.  The three tests are made here, in the parent,
	 * before the fork: a child's only channel back to the user is an exit
	 * status, so a refusal reported from there would arrive as a bare
	 * failure with no reason attached.  The nenvp scan covers the
	 * `PATH=... command' form, whose assignment goes into the child's
	 * environment instead of through setsvar.
	 */
	if (rflag) {
		register char **app2;

		for (app2 = niovp; *app2 != NULL; app2 += 1)
			if (rwrite(*app2)) {
				erestrict(*app2);
				slret = 1;
				return (0);
			}
		for (app2 = nenvp; *app2 != NULL; app2 += 1)
			if (rvar(*app2)) {
				erestrict(*app2);
				slret = 1;
				return (0);
			}
		if (nargc && index(nargv[0], '/') != NULL) {
			erestrict(nargv[0]);
			slret = 1;
			return (0);
		}
