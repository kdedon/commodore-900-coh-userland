...
{
	register char *cmd;
	register int isnewgrp;

	isnewgrp = nargv[0][0] != 'l';
	cmd = isnewgrp ? "/bin/newgrp" : "/bin/login";
	/*
	 * newgrp is refused by a restricted shell.  /bin/newgrp ends by
	 * exec'ing /bin/sh with argv[0] "-" -- on success, and through perrx
	 * on every failure including a group name that does not exist -- so
	 * it is an unrestricted login shell for the asking.  Confining PATH
	 * cannot stop it: this is a builtin, and it runs the program by
	 * absolute pathname without consulting PATH at all.
	 *
	 * login is left alone.  It demands the credentials of whatever
	 * account it is asked for, and it is how a confined user is meant to
	 * end the session.
	 */
	if (rflag && isnewgrp) {
		erestrict(nargv[0]);
...
{
	/*
	 * A restricted shell confines its user to the directories PATH and
	 * $HOME name.  The refusal is here rather than in the cd builtin
	 * because pushd and popd change directory through this routine too.
	 */
	if (rflag) {
		erestrict("cd");
		return NULL;
