...
char **envp;
{
	...
	/*
	 * $VERSION is the system's release, ASKED of the running kernel rather
	 * than compiled in.  A -D put the shell's own build's idea of it in
	 * here, which is a different thing being measured at a different
	 * moment: two binaries in one image reported two stale commits, and
	 * neither was the release the image was.  A kernel that will not answer
	 * leaves $VERSION unset, which is honest; a made-up one is not.
	 */
	{
		struct utsname uts;

		if (uname(&uts) >= 0 && uts.release[0] != '\0')
			assnvar("VERSION", uts.release);
	}
	if (senvp != NULL) {
		vfree(senvp);
	}
...
/*
 * The variables a restricted shell will not let its user set.  PATH decides
 * which program every command name without a `/' in it runs, since ffind
 * searches it, and SHELL names the interpreter that other programs start when
 * they offer a shell escape: setting either one defeats every other
 * restriction.  Those are the only two this shell has that matter -- it reads
 * no ENV, and cd is refused outright, so HOME buys nothing.
 *
 * Takes either a bare name or a `name=value', which is what vareq compares.
 */
rvar(np)
char *np;
{
	return (vareq("PATH", np) || vareq("SHELL", np));
}
...
#endif
	/*
	 * Every assignment reaches this routine -- a plain one, `export
	 * NAME=value', `read NAME' and a `for NAME' loop all arrive through
	 * assnvar or flagvar -- so one test here covers them all.  The
	 * environment the shell inherits is read by initvar before the profile
	 * runs, and therefore before rflag is on.
	 */
	if (rflag && rvar(cp)) {
		erestrict(cp);
		return (NULL);
