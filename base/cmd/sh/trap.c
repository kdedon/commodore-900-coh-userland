	...
	for (sig = 0; sig <= NSIG; sig += 1) {
		if (trap[sig].t_cnt) {
			...
			if (actp = trap[sig].t_act) {
				/*
				 * The action is unhooked for the length of
				 * its own run, so the same signal arriving
				 * inside the handler does not re-enter
				 * session() on top of it.  It goes back
				 * afterwards -- a trap persists until it is
				 * cleared -- unless the handler set a new
				 * one, in which case the new one stands and
				 * the old string is released.
				 */
				trap[sig].t_act = NULL;
				session(SARGS, actp);
