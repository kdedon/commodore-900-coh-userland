...
/*
 * Exit status of a shell that failed itself rather than running something
 * that failed: it could not open the script it was given, or it could not
 * parse its input.  The status of the last command is what a shell normally
 * exits with, and no command runs in either case.
 */
#define SLFAIL	2
...
extern	int	rshpend;		/* Restrict once the profile has run */
