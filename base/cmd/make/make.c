...
/* print lineno, issue warning and RETURN: a warning is not a stop */
#if	USE_PROTO
...
char * name;
{
	...
	/*
	 * lib.a(member.o): the member's insertion date, read out of the
	 * archive.  <ar.h> on this system is the COHERENT one -- an int
	 * magic number, a DIRSIZ name, a time_t date and an fsize_t size --
	 * not the SysV/COFF header of ASCII fields.  The header is stored
	 * CANONICAL, which on this machine means byte-swapped: ar writes it
	 * through canint/cantime/cansize (cmd/ar.c geth/puth) and a reader
	 * that takes the bytes as they lie never matches ARMAG at all.
	 */
	subname = strchr (name, '(');
	if (subname == NULL)
		return 0;
	...
	* lwa = 0;
	result = 0;
	while (read (fd, (char *) & hdrbuf, sizeof hdrbuf) == sizeof hdrbuf) {
		cantime (hdrbuf.ar_date);
		cansize (hdrbuf.ar_size);
		if (strncmp (hdrbuf.ar_name, subname, DIRSIZ) == 0) {
			result = hdrbuf.ar_date;
			break;
		}
		if (lseek (fd, hdrbuf.ar_size, 1) == -1L)
			break;
	}
	* lwa = ')';
	close (fd);
...
#if 0
	if (dflag)
		printf ("fexists (%s) = %d getmdate (name) = %ld\n", name,
...
char * cmd;
{
	...
	do {
		...
		if (rstat != 0 && ! iflg) {
			/*
			 * Not err(): that exits, and -k has to be read after
			 * the command has been reported.
			 */
			fflush (stdout);
			if (sflg)
				fprintf (stderr, "make: %s\texited with "
					 "status %d\n", cmd, rstat);
			else
				fprintf (stderr, "make: \texited with "
					 "status %d\n", rstat);
