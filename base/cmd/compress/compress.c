 ...
 *
 * Usage: compress [-dfvc] [-b bits] [-w workfile] [file ...]
 * Inputs:
 ...
 *
 * Revision history
 * 90/07/11  07:53:00  Charles Fiterman
 * Make compress work for COHERENT
 * This first involved making it work on a system where an int
 * is 16 bits. Then installing a virtualizing system to permit
 * a small model compress.
 */
...
# define MAXCODE(n_bits)	((1L << (n_bits)) - 1)
	...
	int maxbits = BITS;	/* user settable max bits */
#else	/* AZTEC86 */
#ifdef VIRTUAL
/*
 * Virtual memory for compress. Uses hash algorithm
...
/*
 * MBKS IS 511, AND IT IS PINNED AT BOTH ENDS.
 *
 * Upper bound: `data' below is MBKS*VBLK bytes, and the 1985 cc2 keeps each
 * VARIABLE's size in a 16-bit signed int and sign-extends it into the 32-bit
 * BSSD field of the l.out header.  A 32766-byte array is written 0x00007FFE
 * and a 32768-byte one 0xFFFF8000, after which nld refuses the object ("out of
 * space, adding module of 65536 bytes in BSSD").  At the original MBKS 512,
 * `data' is 32768 bytes exactly -- one byte over.  The limit is per variable,
 * not per object: an object's bss total passes 32767 with no trouble.
 *
 ...
 * codetabof (below) address data[0] FLAT as the whole table --
 * HSIZE12*sizeof(count_int) + HSIZE12*sizeof(short) = 30018 bytes.  Anything
 * under MBKS 470 silently truncates the hash table and writes a stream that no
 * decompressor can read; 256 produces plausible, smaller, corrupt output.
 *
 * So MBKS lives in [470, 511], and 511 is the one that gives up the least.
 */
#define MBKS 511		/* number of virtual blocks */

/*
 * NO BIT-FIELDS HERE.  The upstream struct packed these two into one 16-bit
 * word (`unsigned dirty:1; unsigned what_in:15;'), and the 1985 cc2 cannot
 * store to a bit-field: a store to `dirty' rewrites the whole containing word,
 * so it zeroes `what_in', and `dirty |= x' leaves dirty clear while corrupting
 * what_in.  With both effects live, virtual() below records neither residency
 * nor dirt: every slot claims to hold block 0 forever, nothing is ever written
 * back to the scratch file, and every page-in reads a hole and comes back
 * zero.  That is the whole of the -b 13..16 corruption -- the tables silently
 * evaporate, compress emits codes for entries it never really stored, and
 * bytes_out exceeds in_count so it exits 2.
 *
 * Two plain unsigneds cost 1022 extra bytes of bss (map[] goes 1022 -> 2044)
 * and nothing else.  Do not re-pack them.
 */
struct mapper {
	unsigned dirty;
	unsigned what_in;	/* which block is in memory */
};
static unsigned long codedsp, codelim;
static int tmpf, ramsw = 0;
static int maxbits = 12;	/* user settable max bits code */
static char *tmpn = NULL;

static char tmpname[] = "/tmp/czXXXXXX";
 ...
 * On decompress data is only suffix
 *
 * The scratch store holds the hash/code tables, which at BITS=16 are far
 * larger than a single Z8001 data segment.  By default it is an ordinary file
 * in /tmp, unlinked right after it is opened so it disappears on exit; -w lets
 * the user name their own.  Coherent 0.7.3 has no working /dev/ram, so the old
 * ram-disk probe is gone -- but if -w names a device special file we still run
 * the is_fs() safety check and use it raw instead of creat()ing over it.
 */
initV(hsize, codesiz)
long hsize, codesiz;
{
	...
	&& ((sb.st_mode & S_IFMT) == S_IFBLK
	 || (sb.st_mode & S_IFMT) == S_IFCHR)) {
		switch(is_fs(tmpn)) {
		case 1:
			fprintf(stderr, "Possible file system on %s\n", tmpn);
			fprintf(stderr, "Remove file system or use -w workfile\n");
			exit(1);
		case -1:
	...
	if(!ramsw) {
		if(-1 == (tmpf = creat(tmpn, 0600))) { /* rw for owner */
			fprintf(stderr, "Cannot create %s\n", tmpn);
			exit(1);
		}
		close(tmpf);

		tmpf = open(tmpn, 2);	/* open to read and write */
		unlink(tmpn);		/* remove after close */
	}
	if(-1 == tmpf) {
		fprintf(stderr, "Cannot open %s\n", tmpn);
		exit(1);
...
#define htabof(i)	htab[(unsigned long)i]
#define sethtab(i, v)	htabof(i) = v
#define codetabof(i)	codetab[(unsigned long)i]
#define setcodtab(i, v)	codetabof(i) = v
#endif /* VIRTUAL */
#endif	/* AZTEC86 */

#	define HTABSIZE (HSIZE*sizeof(count_int))
#	define CODETABSIZE (HSIZE*sizeof(unsigned short))
...
#else /* normal machine */
#define tab_prefixof(i)	codetabof(i)
# define tab_suffixof(i)	((char_type *)(htab))[(unsigned long)i]
# define setsuffix(i, v) tab_suffixof(i) = v
...
#ifdef VIRTUAL
fprintf(stderr,
  "Usage: compress [-dDVfc] [-b maxbits] [-w workfile] [file ...]\n");
#else
...
#ifdef VIRTUAL
fprintf(stderr,
   "Usage: compress [-dfvcV] [-b maxbits] [-w workfile] [file ...]\n");
#else
...
REGISTER int argc; char **argv;
{
    ...
    *fileptr = NULL;
     ...
     * -w workfile => use workfile for virtual work.
    ...
    for (argc--, argv++; argc > 0; argc--, argv++) 
	{
		...
		{	/* A flag argument */
			...
			{	/* Process all flags in this arg */
			    ...
			    case 'w':
					if(!ARGVAL()) {
					    fprintf(stderr, "Missing workfile\n");
					    Usage();
					    myExit(1);
					}
					tmpn = *argv;
    ...
    if (NULL == (codetab = (unsigned short *)malloc((unsigned)CODETABSIZE)))
    {
        ...
        for (fileptr = filelist; *fileptr; fileptr++) 
		{
			...
			{			/* DECOMPRESSION */
				...
				if (strcmp(*fileptr + strlen(*fileptr) - 2,
					dotz) != 0) 
#else
				if (strcmp(*fileptr + strlen(*fileptr) - 1,
					dotz) != 0) 
#endif
				{
				    /* No .Z: tack one on */
			...
			{					/* COMPRESSION */
				...
				/*
				 * tune hash table size for small files
				 * -- ad hoc,
				...
				if ( (12 >= maxbits) || fsize < (1 << 12) )
				    hsize = min ( 5003, HSIZE );
				else if ( (13 == maxbits) || fsize < (1 << 13) )
				    hsize = min ( 9001, HSIZE );
				else if ( (14 == maxbits) || fsize < (1 << 14) )
				    hsize = min ( 18013, HSIZE );
				else if ( (15 == maxbits) || fsize < (1 << 15) )
				    hsize = min ( 35023L, HSIZE );
				else if ( fsize < 47000L )
				    hsize = min ( 50021L, HSIZE ); /*lint +e506 +e712 */

				/* Generate output filename */
				strcpy(ofname, *fileptr);
#ifndef BSD4_2		/* Short filenames */
				if ((cp=strrchr(ofname,SLASH)) != NULL)
					cp++;
				else
					cp = ofname;
				if (strlen(cp) > 12) 
	...
	{		/* Standard input */
		...
		} else 
		{
		    ...
		    if (nomagic == 0) 
			{
				...
				maxbits &= BIT_MASK;
				maxmaxcode = 1L << maxbits;
				fsize = 100000L;		/* assume stdin large for USERMEM */
				if(maxbits > BITS) 
				{
					fprintf(stderr,
...
void compress() 
{
...
#endif
	{
		...
		/*
		 * (long)c, not c: hshift is 8 whenever hsize >= 65536, which
		 * is exactly the BITS=16 table (HSIZE 69001), and `int' here is
		 * 16 bits.  Left-shifting a byte >= 128 by 8 in an int
		 * overflows to a NEGATIVE value, and `i' is then a negative
		 * hash index -- virtual() shifts it right into a negative block
		 * number, `which' wraps to a huge unsigned, and the run dies
		 * with "Error in write" (or scribbles outside data[]).  A file
		 * of pure 7-bit text never reaches it; any binary does, at
		 * once.  Doing the shift in long keeps i in [0, hsize).
		 */
...
#endif
		{
	 	    setcodtab (i, free_ent++);	/* code -> hashtable */
		    sethtab (i, fcode);
		}
...
void decompress() {
	...
	*stackp++ = finchar = tab_suffixof(code);
	...
	{
	    setcodtab(code, oldcode);
	    setsuffix(code, finchar);
	    free_ent = code+1;
	} 
	/*
...
dump_tab()	/* dump string table */
{
	...
	for(i = first; i < free_ent; i++) {
		...
		fwrite( &stack[stack_top], 1, STACK_SIZE-stack_top, stderr);
...
void onintr ( )
{
    /* No (void) cast: signal() returns a POINTER (int (*signal())(), and the
     * 1985 cc1 has no rule for casting a pointer-valued CALL to void --
     * "no match, op=65", op 65 being Cast.  (void) on an int-returning call is
     * fine, and so is (int) on this one; it is that one combination.  The cast
     * is a lint convention from a decade later and buys nothing here. */
...
{
	signal(SIGSEGV,SIG_IGN);		/* see onintr: (void) on a pointer call */
    if ( do_decomp == 1 ) 
