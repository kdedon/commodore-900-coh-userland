...
#define	MAXNBUF	200		/* Maximum namebuf length		*/
...
fsize_t dirsize;
{
	...
	if (!fflag) {
		...
		dsaved = alloc((unsigned)size,"dsaved");
		dsavep = dsaved;
	}
	if (Rflag)
	...
	for (dp=dirbuf; dp<(dirbuf+nb); dp++) {
		...
		*np2 = '\0';
		if (lflag || xflag || Cflag || sflag || tflag || Rflag ||
			...
			if (strlen(dir) + DIRSIZ + 2 > MAXNBUF) {
