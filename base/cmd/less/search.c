...
#if REGCOMP
/*
 * The regexp package reports a bad pattern by calling this and then
 * returning NULL from regcomp().
 */
	public void
regerror(s)
	char *s;
...
#else
	if ((parg.p_string = regcomp(pattern)) != NULL)
#endif
	{
		error("%s", &parg);
		return (-1);
