...
CH:
...
#ifdef ALTERNATE_KEYS
		case 'g':
			if (strchr("hjklyubn", c = rgetchar()) == NULL) {
			   message("bad direction");
			   break;
			}
			if (islower(c)) c = toupper(c);
			multiple_move_rogue(c);
			break;
#endif
		case 'e':
			eat();
...
#ifdef MSDOS
			message("DOS version 1994 by Nils M. Holm (nmh@sunrise.rni.sub.org)", 0);
#else
			message("minor improvements 1993 by Nils M. Holm (nmh@sunrise.rni.sub.org)", 0);
#endif
			break;
		case 'Q':
			quit(0);
		...
		default:
#ifdef ONLINE_HELP
			message("Unknown command - press '?' for help");
#else
			message(unknown_command, 0);
#endif
			break;
