	...
	if (eptr = md_getenv("ROGUEOPTS")) {
		for (;;) {
			...
			if (!strncmp(eptr, "fruit=", 6)) {
				...
				show_skull = 0;
#ifdef NEWOPTS
			} else if (!strncmp(eptr, "flush", 5)) {
				kb_flush = 1;
			} else if (!strncmp(eptr, "grafix", 6)) {
				grafix = 1;
#endif
			}
...
#ifdef UNIX
char *my_tgetstr(id, tdef)
char  *id, *tdef;
{
   static char  ctlstr[32];
   char   tid[4];
   int   i = 0;
   char   *p;
...
init_keys()
{
   ...
   if ((termtype = getenv("TERM")) == NULL)
	clean_up("cannot get terminal type");

   if (tgetent(tcap, termtype) != 1)
	clean_up("unknown terminal type");

   for (i=0; i<4; i++)
	strcpy(arrows[i], my_tgetstr(tcid[i], tcap));
}
#endif	/* UNIX */
