...
tgetch()
{
   ...
   char  sbuf[20];
#ifdef NEWOPTS
   extern boolean	kb_flush;
#endif

#ifdef NEWOPTS
   if (kb_flush) md_slurp();
#endif
   ...
   for (l=i=0; !alarm_done; ) {
	c = getchar();
	sbuf[++l] = c;
	sbuf[l+1] = 0;

	if (!strcmp(arrows[0], sbuf)) {
	   alreset();
	   return('k');
	}
	else if (!strcmp(arrows[1], sbuf)) {
	   alreset();
	   return('j');
	}
	else if (!strcmp(arrows[2], sbuf)) {
	   alreset();
	   return('h');
	}
	else if (!strcmp(arrows[3], sbuf)) {
	   alreset();
	   return('l');
	}

	for (ok=i=0; i<4; i++)
	   if (!strncmp(arrows[i], sbuf, strlen(sbuf)))
		ok = 1;
...
tgetch()
{
	...
	switch (c) {
		case 328:	return(shifted()? 'K': 'k');
		case 336:	return(shifted()? 'J': 'j');
		case 331:	return(shifted()? 'H': 'h');
		case 333:	return(shifted()? 'L': 'l');
		default:	return(c);
	}
}
#endif /* MSDOS */

#endif /* ALTERNATE_KEYS */
