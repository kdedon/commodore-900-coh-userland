 ...
 *  stored in the file. If the open fails, abort. If the read fails,
 *  abort. If the read PID does not match our PID, abort. We will only
 *  remove the lock if the passed pid matches the PID written to the file.
 *
 *  A pid of 0 is the wildcard: it removes the lock whoever owns it.  init(1)
 *  uses it (unlockntty("console", 0)) to clear locks left by logins that never
 *  logged out.  No process has pid 0, so a real owner is never matched by it.
 */
