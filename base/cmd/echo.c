...
register char	** argv;
{
	...
	/*
	 * argv[1] may be NULL, and the test for -n must not assume otherwise.
	 * `echo' with no arguments is ordinary -- a script writes it for a
	 * blank line, and any unset variable produces it: `echo $x' with x
	 * unset expands to nothing at all, so the shell execs echo with
	 * argc == 1.
	 *
	 * The original dereferenced argv[1] unconditionally.  That survives on
	 * a machine where address zero reads as zero, because strcmp then just
	 * compares 0 against '-' and returns non-zero; here segment 0 offset 0
	 * is not mapped and it is a segmentation violation instead.  Same
	 * source, same bug, different consequence -- which is why it arrives
	 * as a surprise in a port rather than as a known defect.
	 */
	++argv;					/* step over the program name */
	if ( *argv != (char *)0 && strcmp( *argv, "-n" ) == 0 ) {
		lastch = 0;
		++argv;
	}
