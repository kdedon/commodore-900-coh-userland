...
/*
 * Act on the signal flags at mainline level.  The handlers only set a
 * flag: stdio must not be entered from signal context, because the
 * signal usually interrupts the mainline inside a write to the printer,
 * and an fprintf there re-enters the half-updated stdio state.  That
 * corrupts the malloc arena and kills the daemon in free(), leaving a
 * stale `dpid' lock behind.  Called from the printing loops -- the
 * control file scan, the file printer, the banner rows -- so a cancel
 * takes effect promptly even in the middle of a long listing.
 */
chksig()
{
	if (gotcancel) {
