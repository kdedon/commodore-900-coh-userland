...
/*
 * The copy buffer.  BUFSIZ is the filesystem block size, which is the unit the
 * loop below works in whatever the buffer holds: a hole is a block of zeros, so
 * the scan is per block, and every write is one block.
 */
char	buf[BUFSIZ];
int	dflag;
char	*namebuf;
