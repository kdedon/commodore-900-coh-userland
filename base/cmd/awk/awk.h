 ...
 * not used here).  The fields live in a named
 * sub-struct (t_b) so a NODE union can expose
 * them too -- K&R cc has no anonymous members.
 */
struct	t_body {
	CHAR	t_op;			/* Op type */
	CHAR	t_flag;
...
/*
 * Expression tree node.  Fields live in a named
 * sub-struct (o_b) for the same reason as t_body.
 */
struct	o_body {
	CHAR	n_op;
	CHAR	n_flag;
...
/*
 * Field aliases.  These are self-referential macros: the field
 * name reappears in the expansion, but CPP does not re-expand a
 * macro inside its own expansion, so each accessor keeps working
 * unchanged on a TERM, an OPNODE, or a NODE union.
 */
#define	t_op	t_b.t_op
#define	t_flag	t_b.t_flag
#define	t_next	t_b.t_next
#define	t_hval	t_b.t_hval
#define	t_ahval	t_b.t_ahval
#define	t_un	t_b.t_un
#define	t_name	t_b.t_name
#define	n_op	o_b.n_op
#define	n_flag	o_b.n_flag
#define	n_o1	o_b.n_o1
#define	n_o2	o_b.n_o2
#define	n_o3	o_b.n_o3
#define	n_o4	o_b.n_o4
