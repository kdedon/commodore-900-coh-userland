 ...
 * three consoles -- no display, low resolution, hi resolution -- that kernel
 * was configured with).  A terminal on this major is a keyboard and a screen
 * attached to this machine; every other line is somewhere else: major 5 is an
 * SCC serial port and major 9 a pseudo-tty, which is what telnetd(1M) hands a
 * network login.  lock.c keeps the PE driver's major the same way.
 */
	...
	{  if (argc > 1)
	   ...
	   if (pwp != NULL)		/* if entry found */
	   {
		...
		goto again;
	      ...
	      /*
	       * The super user is a CONSOLE account.  Its passwd entry has no
	       * password -- deliberately, so that a machine whose only input
	       * is the keyboard in front of it is never locked out of itself --
	       * and an empty field is entered with no credential at all (the
	       * getpass block below is skipped).  That is safe for a terminal
	       * somebody is standing at and nowhere else, so uid 0 is refused
	       * on any line that is not one of this machine's own consoles,
	       * whatever the password field holds.  Every network login path
	       * arrives on a pseudo-tty (telnetd(1M) allocates one and runs a
	       * getty on the slave), and a serial port is not the console
	       * either, so both are refused here.
	       */
	      if (pwp->pw_uid == 0 && !isconsole(2)) {
		 setutmp(s_tty, buff, faillog, FALSE);
		 printf("%s may log in on the console only\n", pwp->pw_name);
		 goto again;
	      }
...
/*
 * Return TRUE if the terminal open on `fd' is one of this machine's own
 * consoles.  The device, not the name, is what is asked: a node called
 * /dev/console that some other driver answers is not a console, and a second
 * node made on the console driver is one.
 *
 * A failed fstat(2), or a descriptor that is not a character device, answers
 * "not the console": this is the test a login is refused on, so anything it
 * cannot establish has to count against the login.
 */
isconsole(fd)
{
	struct stat sbuf;
	...
	if ((sbuf.st_mode&S_IFMT) != S_IFCHR)
		return (FALSE);
	return (major(sbuf.st_rdev) == KVCON_MAJOR);
}
