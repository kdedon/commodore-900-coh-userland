...
FILE *tfp;
{
	...
	for (ulist = users; *ulist != NULL; ulist++) {
		...
		if (index(aftralias, '@') != NULL) {
			if (ssend(aftralias, tfp) != 0)
				senderr = 1;
			continue;
		}
...
/*
 * Deliver to user@host over SMTP: the network counterpart of rsend(), taking
 * the same seam and the same shape.  smtpsend(1) reads the whole message --
 * the headers build_header() writes and then the body -- on its standard
 * input, opens the connection and speaks RFC 821.  It has no queue, so a
 * non-zero exit is a permanent failure and the letter goes to dead.letter.
 */
ssend(who, tfp)
char	*who;
FILE	*tfp;
{
	...
	sprintf(cmdname, "smtpsend -f %s %s", myname, who);
	if (verbflag)
		fprintf(stderr, "Sending mail to %s over SMTP\n", who);
	if ((xfp = popen(cmdname, "w")) == NULL) {
		mmsg("Can't pipe to %s\n", cmdname);
		return 1;
	}
	if (build_header(who, NULL, tfp, xfp) != 1
	  || mcopy(tfp, xfp, ftell(tfp), (fsize_t)MAXLONG, 0)) {
		merr(wrerr, cmdname);
		pclose(xfp);
		return 1;
	...
	if ((i = pclose(xfp)) != 0) {
		mmsg("smtpsend has failed, status %d\n", i);
		return 1;
	}
	return 0;
}
...
struct	tm	*tp;
{
	...
	/* struct tm counts years from 1900, so the year is written in full:
	 * `22 Jul 2026', the four-digit form RFC 1123 asks for. */
...
struct	tm	*tp;
{
	...
	/* The date part of a Message-Id, YYYYMMDDHHMM: the four-digit year
	 * keeps ids from two centuries apart in ascending order. */
	sprintf(msgidbuf, "%04d%02d%02d%02d%02d",
