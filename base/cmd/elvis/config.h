...
/* USE_TERMCAP selects the real termcap database (tgetent() et al from
 * /usr/lib/libterm.a, reading /etc/termcap) over the compiled-in ANSI
 * description in tinytcap.c.  With tinytcap the screen size is a constant
 * 25x80 no matter what TERM says, so a terminal of any other size is used
 * only in part; the C900 hi-res console is 85x32.  Elvis' own Makefile picks
 * termcap for Coherent (LIBS= -lterm, EXTRA= empty), and only MS-DOS, TOS and
 * Minix-PC compile tinytcap.  The build sets -DUSE_TERMCAP=1 in the same place
 * it adds libterm.a to the link, so the two always agree.
 */
#ifndef USE_TERMCAP
...
#ifndef COHERENT
# define COHERENT 1		/* Coherent (the C900 port: set here instead
				 * of -D so cc0 sees one config point) */
# define CRUNCH 1
# define NO_CHARATTR 1
# define NO_CURSORSHAPE 1
# define NO_DIGRAPH 1
# define NO_MKEXRC 1
#endif
 ...
 */
#if BSD || UNIX7 || OSK || COHERENT	/* the 0.7.3 libc has index(), not strchr() */
# define strchr	index
