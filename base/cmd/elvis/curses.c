...
#if USE_TERMCAP
extern short	ospeed;	/* libterm's tputs() owns this one */
#else
...
char	PC;		/* Pad char */
# if USE_TERMCAP
char	*BC;		/* :bc=: backspace string; libterm's tgoto() reads it */
# endif
#endif
...
#if USE_TERMCAP
	/* libterm reads PC and BC itself: tputs() sends PC as the pad
	 * character, and tgoto() backs the cursor up with BC when a cursor
	 * address would otherwise contain a control character.  A program
	 * that links libterm has to define both -- the library only declares
	 * them.  PC = NUL and BC = 0 (meaning \b) are the right defaults.
	 */
	if (str = tgetstr("pc", &capbuf))
		PC = *str;
	MAYHAVE(BC, "bc");
#endif
