...
#include "config.h"
/* USE_TERMCAP means libterm.a supplies these functions from /etc/termcap, and
 * this file must define nothing at all -- two definitions of tgetent() would
 * not link, and the tiny one answers 25x80 for every terminal.
 */
#if (MSDOS || TOS || MINIX || COHERENT) && !USE_TERMCAP
	...
	char	**bp;	/* pointer to pointer to buffer - ignored */
{
	switch (CAP(id))
	{
	  ...
	  case CAP2('P','U'):	return "\033[V";
	  case CAP2('P','D'):	return "\033[U";
	  case CAP2('H','M'):	return "\033[H";
# if MINIX
	  case CAP2('E','N'):	return "\033[Y";
# else /* COHERENT */
	  case CAP2('E','N'):	return "\033[24H";
# endif
#else /* MS-DOS or TOS */
