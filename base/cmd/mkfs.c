...
extern ino_t ialloc();
extern char *bcache();
/* malloc() and realloc() return a far pointer; undeclared they would default
 * to int, which is half a pointer here. */
extern char *malloc();
extern char *realloc();
...
getsuper()
{
	...
	time(&S.s_time);
	/* A freshly made file system is clean: mount(2) forces a file system
	 * whose s_dirty is not FSCLEAN read only. */
	S.s_dirty = FSCLEAN;
	return (0);
}
...
nextname:
	...
	while (--nent >= 0) {
		...
		/*
		 * The name matches this entry when the two end at the same
		 * character.  The name's component ends at NUL or at '/';
		 * the entry's name ends at ITS NUL or at DIRSIZ, which is
		 * all a directory record holds.  Both ends are folded to 0
		 * so one comparison decides it, and the walk stops at the
		 * shorter of the two -- a loop that runs past the end of
		 * either reads a byte that is not part of the name it is
		 * matching and can take the wrong entry.
		 */
		for (i = 0; ; i += 1) {
			register int nc, ec;
			...
			if (nc != ec)
				break;		/* not this entry */
			if (nc == 0) {		/* both ended here */
				inum = ep->e_ino;
				assert(inum > BADFIN);
				assert(inum <= nino);
				if (name[i] == 0)
...
/*
 * An entry holds a block number within one interleave cycle, so the largest
 * is S.s_n-1 and the table has to be wide enough for it.
 */
#define	MAXINTN	500
