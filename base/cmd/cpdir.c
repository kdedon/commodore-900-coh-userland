 ...
 *	  SLOW selects the simpler copy loop: it tests the whole buffer for a
 *	  hole in one pass instead of testing each block of it.
 */
#include <stdio.h>
#include <errno.h>
#include <signal.h>
...
#define	VERSION		"1.1"
/* The copy buffer, in filesystem blocks.  BUFSIZ is the block size, which is
 * the unit cpfile() works in whatever the buffer holds: a hole is a block of
 * zeros, so the scan is per block and every write is one block.  A buffer of
 * many blocks changes only how much one read() fetches, and CPBLOCKS is the
 * number it fetches; -DCPBLOCKS=1 gives a buffer of one block. */
#ifndef	CPBLOCKS
#define	CPBLOCKS	50
#endif
#define	CPBUFSIZ	(CPBLOCKS*BUFSIZ)		/* copy buffer */
