 ...
 * The year field holds the last two digits of the year: every field of an
 * RCS date is exactly two characters wide, and PRINTDATE/PRINTTIME index the
 * later fields by fixed offset.
 */
...
#if defined(COHERENT)
#define	putc(c,fp)	(--(fp)->_cc<0 ? ++(fp)->_cc,fflsbuf((c),(fp)) \
				       : (*(fp)->_cp++=(c)))
#else
...
#if defined(USG) || defined(V4_2BSD) || defined(COHERENT)
#define VOID	(void)
#else
typedef int void;
#define VOID
