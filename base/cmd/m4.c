...
typedef	struct {		/* for storing variable-length strings */
	...
	/* Unsigned so that a name long enough to overflow a 16-bit int still
	 * yields a bucket index inside e_root[]. */
	unsigned s_hash;	/* raw hash value (sum of chars in body) */
	char	*s_body;	/* start of contents */
	char	*s_next;	/* end of contents */
	char	*s_last;	/* end of storage */
} STRING;

typedef	struct	ifr {			/* input-source stack frame */
	struct	ifr *i_back;
	char	i_cbuf;			/* unget buffer for lookahead */
	int	i_type;			/* MSTRing or DSKFile */
	union {
		struct {		/* i_type == MSTR */
			STRING	*u_pstr;
			char	*u_pchr;	/* next char */
		} u_str;
		FILE	*u_fp;		/* i_type == DSKF */
	} i_u;
} IFRAME;
#define	i_pstr	i_u.u_str.u_pstr
#define	i_pchr	i_u.u_str.u_pchr
#define	i_fp	i_u.u_fp
