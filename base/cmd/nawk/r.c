...
/*
 * MAXREC bounds one input record.  get1rec() below reads a record into a
 * caller's buffer with no check of its own; every such buffer in this program
 * -- record[] here, the getline target in Getline(), and the field buffers in
 * mkfld()/r_mkfld() -- is BUFSIZ bytes, and a record longer than that walks off
 * all of them.  BUFSIZ is 512 here and was larger on Minix, which is why the
 * authors never met it.  A record is refused rather than truncated: truncation
 * would answer a question about a file with an answer about part of it.
 */
#define MAXREC	BUFSIZ
...
get1rec(buf, fp) char *buf; FILE *fp;
{
  ...
  if (feof(fp) || (c = getc(fp)) == EOF)
	return 0;
  lim = buf + MAXREC - 1;		/* room for the '\0' */
  for (s = buf; ; ) {
