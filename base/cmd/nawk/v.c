...
/*	if (u->c_fval >= -2147483648.0 && u->c_fval <= 2147483647.0)*/
	/*
	 * An integral value prints as an integer.  The original asked
	 * printf for "%.16g", which assumes 17 correctly-rounded
	 * significant digits; this libc's %g does not deliver them, and
	 * `print 99' answered 98.9999999999999860.  The value is known to
	 * fit a long on the line above, so print it as one -- exact, and
	 * cheaper than the floating conversion it replaces.
	 */
	if ((long)u->c_fval == u->c_fval)
		sprintf(str, "%ld", (long)u->c_fval);
	else
		sprintf(str, *OFMT, u->c_fval);
	sfree(u->c_sval);
