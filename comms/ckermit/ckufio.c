...
#include <sys/filsys.h>			/* File access */
#endif
#endif
#endif
#endif
...
zchout(n,c) int n; char c; {
    char cb;				/* write(2) wants the byte in memory: a local
					   is one whatever the ABI does with a char
					   parameter, which `&c' depends on */
    if (chkfn(n) < 1) return(-1);
    if (n == ZSFILE) {
	cb = c;
    	return(write(fileno(fp[n]),&cb,1)); /* Use unbuffered for session log */
    }
...
char *p;
{
 ...
 while (*p != '\0')
 {
   ...
   debug(F101,"splitpath malloc","",(int) cur);	/* F101's n is an int */
...
int len;
{
 ...
 while (head != NULL) {			/* return the path segments */
   struct path *next = head -> fwd;	/* read the link BEFORE freeing it */
   free(head);
   head = next;
 }
...
char *sofar,*endcur;
{
/* COHERENT reads a directory the V7 way -- open(2) it and read(2) its fixed
 * 16-byte struct direct records, which is what ls(1) itself does -- so it takes
 * the second arm below and needs no directory library.  There is no opendir()
 * arm for it to take: <sys/dir.h> here has no DIR type (only `#define DIR
 * char *') and declares no readdir(), and an undeclared pointer-returning
 * function is an int by K&R default, which a 32-bit far pointer does not
 * survive.
 */
#ifdef BSD42
 DIR *fd, *opendir();
 ...
 *endcur = '\0';                        	/* end current string */
 ...
 while ( read(fd,dirbuf,sizeof dir_entry) == sizeof dir_entry )
#endif
{
