...
#include <errno.h>
/*
 * strchr() returns a pointer.  Without a declaration K&R gives it int, which
 * on a 32-bit-pointer target keeps only the low half of the address, so
 * makedirs() below walks a truncated path pointer.
 */
#include <string.h>
extern int errno;
FILE *popen();
