...
# away from root.
#
# emu-run.sh logs in as root, so each case is a nested su: the outer drops to
# guest and the inner one is under test.
#
...
#
# A T_OFF that let su through leaves ROOTOK in its file instead of the refusal.
#% needs runtime base
#% expect ^ROOTOK$
