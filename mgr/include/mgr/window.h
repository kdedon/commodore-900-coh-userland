...
#define M_NOREPORT	15	/* don't tell kernel our window size */
#define M_CONSOLE	16	/* redirect console to this device */
...
#define P_BITOP		0x800	/* push bitblit ops */


#define P_ALL		0xfff	/* push everything */
#define P_MAX		0x1000	/* end of codes marker */
...
#define E_BITVALUE      'O'     /* get/set pixel value */
...
#define B_DST BIT_DST
#define B_OR BIT_OR
#define B_COPY BIT_SRC
#define B_COPYINVERTED BIT_NOT(BIT_SRC)
#define B_XOR BIT_XOR
#define B_AND BIT_AND
#define B_SET BIT_SET
#define B_CLEAR BIT_CLR
#define B_INVERT BIT_INV
#endif

/* other macros */

/*
 * Map a virtual graphics coordinate (0..GMAX) onto a pixel of the window.
 * The product of a coordinate and a window dimension reaches 1024000, so it
 * is formed in long: on a 16-bit int it would otherwise wrap and the clipper
 * would collapse the whole drawing onto one edge of the window.
 */
#define Scalex(x) \
	(W(flags)&W_ABSCOORDS ?  (x) : \
	 (int)((long)(x) * (long)BIT_WIDE(W(window))/GMAX))
#define Scaley(y) \
	(W(flags)&W_ABSCOORDS ?  (y) : \
	 (int)((long)(y) * (long)BIT_HIGH(W(window))/GMAX))
#define Scalexy(y) \
	(W(flags)&W_ABSCOORDS ?  (y) : \
	 (int)((long)(y) * \
	  (long)(BIT_HIGH(W(window))+BIT_WIDE(W(window)))/(2*GMAX)))
