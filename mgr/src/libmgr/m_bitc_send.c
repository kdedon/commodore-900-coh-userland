...
int m_bitc_sendh(w, h, d, to, x, y, to_tmp)	/* client -> server bitmap */
int w;			/* width in bits */
...
int to_tmp;		/* (free) temp bitmap, unused if x=y=0 */
{
	...
	if( filep) {
	    ...
	    setbuf( filep, ttybuff);	/* no setvbuf/_IOFBF in this stdio */
 ...
 */
int m_bitc_sendb(data, len)
char *data;		/* start of portion of bitmap data */
unsigned int len;	/* length in bytes of portion */
{
