...
int main(argc,argv) int argc; char **argv;
{
  ...
  get_param(mgr_host,&xmax,&ymax,(int*)0);
  ...
  while(m_gets(buff)) 
  {
    ...
    switch (*buff) 
    {
      ...
      if (tmp && !snapping) 
      {
        ...
        /* Discard the previous snapshot: this system has no ftruncate(2),
           and creat() on the open file's name empties it in place, so the
           rewrite from offset 0 leaves no tail of the older, larger image. */
        fflush(fp);
        if ((tfd = creat(name,0666)) >= 0)
          close(tfd);
        fseek(fp,0L,0);
        if (!bitmapwrite(fp,tmp) )
        {
          m_push(P_ALL);
          m_font(0);
          m_size(27,3);
