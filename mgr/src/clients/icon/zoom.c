...
/* zoom uses sprintf() for its value, which this stdio already supplies:
   <stdio.h>:75 declares `char *sprintf()' returning its own first argument.
   The upstream vsprintf() wrapper that supplied those semantics elsewhere is
   therefore not needed, and there is no vsprintf() here to build it from. */
...
char **argv;
{
   ...
   int lastx, lasty = 0;	/* prev. bit position in bitmap */
   int done=0;			/* exit flag */
   int code = 0;		/* return codes */
   ...
   if ((map = read_icon(argv[1],0))  == BIT_NULL) {
      ...
      bit_blit(map,0,0,w,h,BIT_CLR,BIT_NULL,0,0);
      }
           ...
           if (function) {
              ...
              switch(function) {
                 ...
                 case GROW:
                    ...
                    bit_blit(temp,0,0, w,h,BIT_CLR,BIT_NULL,0,0);
              ...
              if (new && mode==TOGGLE) {
                 ...
                 bit_blit(new,0,0,W(new),H(new),
                          mode==SET?BIT_SET:BIT_CLR,BIT_NULL,0,0);
                 }
              if (new)
                 bit_destroy(new);
              }
