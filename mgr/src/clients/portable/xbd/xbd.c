...
#endif
#ifdef MGR
/* One tick of the game clock.  There is no interval timer on COHERENT, so
   the main loop waits for a key with this as its select() timeout and treats
   the timeout as the tick; tick_armed carries the state the SIGALRM
   disposition carries under X11 -- ticks run only while it is set. */
...
#endif
#ifdef MGR
  cycletime.tv_sec = period / 1000000;
  cycletime.tv_usec = period % 1000000;
#endif
}
  ...
  char          **argv;
{
  ...
  if (speed <= 0) speed = 1;
  period = (long) 3 *(long) 625000 / speed;
  /* start the game clock.  the tick is not delivered until the first */
  ...
  while (lives > 0)		/* MAIN LOOP */
  {
    ...
    FD_SET(0,&keyfds);
    tick = cycletime;
    ready = select(1,&keyfds,(fd_set *)0,(fd_set *)0,
		   tick_armed ? &tick : (struct timeval *)0);
    if (ready == 0)
      ticker(0);
    else if (ready > 0 && read(0,&c,1)==1)
    {
      tick_armed = 0;
      keyhit=c;
	switch (keyhit)
