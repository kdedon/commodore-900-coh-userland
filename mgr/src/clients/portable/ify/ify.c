...
as a title for the closed window, and then stopping output on it with
tcflow(TCOOFF), basically equivalent to a ^S.  After sending the ^S,
ify sends the escape codes to restore the window's position and
content; since the tty has been ^S'ed, these don't take effect yet.
...
ptty, and restarts its output with tcflow(TCOON) (equivalent to a ^Q).
This lets MGR see the escape codes ify queued up when the window was
...
#include <errno.h>
#include <unistd.h>
#include <termios.h>		/* for tcflow(), TCOON and TCOOFF */
#include <mgr/mgr.h>
...
/* ify's icon stuff: */
#define MGRICONNAME "ify"	/* name for global mgr icon */
...
int disp_w, disp_h, disp_d;	/* display size */
...
#   define BUFLEN (80)
    ...
    m_getscreensize(&disp_w,&disp_h,&disp_d);
    border=m_getbordersize();
    m_getfontsize(&font_w, &font_h);
    font = get_font(&font_w, &font_h);
    DUPKEYON;
    ...
    DUPKEYOFF;
    m_getwindowposition(&wind_x, &wind_y);
    m_getwindowsize(&wind_w, &wind_h);
    DUPKEYON;
    if (shapetofit) {
   ...
   that position for free */
{
    ...
    DUPKEYOFF;
    m_getwindowposition(&wind_x, &wind_y); /* where are we? */
    DUPKEYON;
	...
	if (strncmp(notify, closednotify, CLNLEN) == 0)
	  openwindows(tty);
	    ...
	    if (tcflow(ttyfd,TCOON) == -1)
	      closedown(THEERROR("tcflow TCOON II: "));

	    /* don't notice the windows that haven't been closed yet */
	    foreachwindow(layout, layoutlen, tty, ttyfp, dontnotice, CNULL);

	    /* close the windows attached to this tty */
	    foreachwindow(layout, layoutlen, tty, ttyfp, shapeclose, notify);

	    /* We need to know when MGR is finished interpreting all those
	       commands we've queued up there so we don't stop the line too soon
	       ...
	       stop it.  windowpid is set iff we could set up the tty to have
	    ...
	    if (tcflow(ttyfd, TCOOFF) == -1)
	      closedown(THEERROR("tcflow TCOOFF: "));
	    
	    /* queue escape codes to re-open them when output is
	       re-enabled */
	    foreachwindow(layout, layoutlen, tty, ttyfp, queueopen, CNULL);
   ...
   this window, record its new position in layout[]. */
{
	...
	m_setmode(M_NOREPORT);	/* turn off kernel window size reporting */
	m_font(font);		/* use ify's font */
...
openwindows(tty)
     char *tty;
/* Do the stuff to open a window- do a tcflow(TCOON) to let the
   commands sent by queueopen() take effect, and tell ify when it's done. */
{
    ...
    if (tcflow(ttyfd, TCOON) == -1)
      closedown(THEERROR("tcflow TCOON: "));
...
void
foreachwindow(win, count, tty, open_fp, func, data)
...
/* find every window in win[0..count-1] attached to the given tty, and
   ...
   open_fp is a stream already open on that tty, or NULL to have one opened
   here and closed again.  A caller that has stopped output on the line must
   supply it: opening the slave end of a pty is what tells this kernel the
   line has just acquired carrier, and that clears the stop.  ptyopen() drops
   T_STOP and ttopen() zeroes t_xflags (sys/drv/pty.c, sys/drv/tty.c), so a
   second open of an iconified window's tty would let the queued restore
   codes through at once and the icon would spring back open.  open_fp is
   only meaningful when tty is given; with tty==NULL this walks several ttys
   and opens each in turn.
   ...
   and data is the same value passed to foreachwindow(). */
{
    ...
    if (tty) {
	if ((ttyfp = open_fp) == NULL
	    && (ttyfp = fopen(FULLTTYNAME(tty), "r+")) == NULL)
...
getwid()
{
    ...
    sprintf(name, "getwid %d", (int)getpid());
    m_push(P_EVENT);
...
/* The menu holds one entry per closed window, and three ceilings bound how
   many there can be.  The server will not create a 25th window at all
   (MAXWIN, src/mgr/defs.h:18, enforced in new_window()), which is the
   smallest of them and so the one that counts.  The other two are looser: a
   menu is clipped to MAX_LIST 100 choices by menu_define() (get_menus.c:34)
   and, on the 1024x800 board, to the (800 - 2*MENU_BORDER)/(font.high+2)
   bars that fit down the screen -- 52 at a 13-high font, less ify's own four
   header rows, i.e. 48.  Windows are the binding constraint at 24.

   The dimensions matter because int is 16 bits: at upstream's 500 all three
   arrays below compute past 32,767 and come out negative.  They are static
   as well as small because two of them were automatic, and 55 KB of one
   stack frame is more than this machine's stack. */
#define MAXITEMS (24)
   ...
   the name of the ptty running the window. */
{
    ...
    static char text[MAXITEMS*MAXTEXTLEN]; /* text of menu items */
    static char events[MAXITEMS*MAXEVENTLEN]; /* text of events */
	...
	if (strncmp(windowname, closednotify, CLNLEN) == 0) {
		...
		for (p = windowname; (p = strchr(p, ',')) != NULL;)
		  *p = ' ';
	    }
