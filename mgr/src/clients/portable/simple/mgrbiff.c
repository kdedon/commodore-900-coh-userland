...
/*{{{  #defines*/
#define POLL 30
/* Where mail(1) delivers on this system -- cmd/mail/mail.c's SPOOLDIR.  The
   name is only the last resort: argv[1], then $MAIL, then MAILBOX/login. */
...
int main(argc,argv) int argc; char *argv[];
{
  ...
  else if (getenv("MAIL")) strcpy(mailbox,getenv("MAIL"));
