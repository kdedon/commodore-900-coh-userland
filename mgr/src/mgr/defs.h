...
#define CLEAR(s,op)	bit_blit((s),0,0,BIT_WIDE(s),BIT_HIGH(s),op,BIT_NULL,0,0);
...
/* Escape sequence parameters.
   ...
   Every number in an escape sequence comes from the client's output stream,
   is signed, and is accumulated one digit at a time by put_window().  Three
   kinds of parameter must be told apart:

     - an INDEX into a per-window array (menus[MAXMENU], bitmaps[MAXBITMAPS],
       events[MAXEVENTS], esc[MAXESC], fontlist[MAXFONT]).  A parameter used
       this way must be range checked in BOTH directions before it indexes
       anything: a negative index reaches back into the window structure
       itself and a too-large one runs off the end of it.  Use ESC_INDEX()
       for a 0-based index and ESC_INDEX1() where number 0 names the window
       and 1..limit name array entries 0..limit-1 (bitmaps and fonts).

     - a LINK value, which is an index or -1 for "no link" (the child menu
       of m_linkmenu()/m_unlinkmenu(), menu_state.next).  It is stored, not
       dereferenced; only its upper bound is checked here, and the >= 0 test
       happens where it is finally used as an index.

     - a BIT MASK, which is legitimately negative because bit 15 is set:
       m_push() sends mode|P_CLEAR, i.e. 0x8000, which prints as a negative
       number and comes back through E_MINUS.  Masks are not bounded.

   ESC_PARAM_MAX/MIN are the range a parameter may hold.  They are the
   target's 16-bit int range rather than INT_MAX/INT_MIN so that the parser
   saturates identically on the target and on a wider host: a client sending
   more digits than fit must not wrap a large positive number into a
   negative one that then passes for an index.

   The arguments of these macros are evaluated more than once.
*/

#define ESC_PARAM_MAX	32767			/* target int range */
#define ESC_PARAM_MIN	(-ESC_PARAM_MAX-1)
#define ESC_INDEX(v,limit)	((v) >= 0 && (v) < (limit))
#define ESC_INDEX1(v,limit)	((v) > 0 && (v) <= (limit))
