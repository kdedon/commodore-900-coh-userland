...
void down_load(win,window,text) register WINDOW *win; BITMAP *window, *text;
{
  ...
  switch(W(code)) 
  {
    ...
    case T_MENU:
    {
      struct font *f;
      int fn = W(esc[1]);
      /* The sign of the menu number is the button it is bound to (E_MENU in
         put_window.c reads it as ABS), not part of the number.  ABS() of the
         most negative int is still negative, so the index must be checked in
         both directions and not just against MAXMENU. */
      int mn = ABS(*W(esc));

      if (!ESC_INDEX(mn,MAXMENU))
      break;
      ...
      if (*W(snarf)) 
      {
        W(menus)[mn] = do_menu(W(snarf),f,W(style));
        if (active == win ) 
        {
          if (W(menu[0]) == mn && button_state==BUTTON_2)
          go_menu(0);
          else if (W(menu[1]) == mn && button_state==BUTTON_1)
          go_menu(1);
        }
      }
      else
      W(menus)[mn] = (struct menu_state *) 0;
    }
    break;
    /*}}}  */
    ...
    case T_STRING:
    {
      ...
      dbgprintf('y',(stderr,"%s: drawing [%s] to %d\r\n",
		    W(tty),W(snarf),*W(esc)));
      /* bitmap 0 means "draw into the window"; any other number must be a
         legal bitmap.  A negative number used to fall through to the window
         as well, which worked by accident of short circuiting. */
      if (*W(esc) != 0 && !ESC_INDEX1(*W(esc),MAXBITMAPS))
      break;
    ...
    case T_GMAP:
    {
      ...
      if (c == '/' || (c == '.' && W(snarf)[1]=='/')) strcpy(filename,W(snarf));
      else sprintf(filename,"%s/%s",icon_dir,W(snarf));

      if (W(flags)&W_DUPKEY) sprintf(buff,"%c ",W(dup)); else *buff = '\0';

      if (ESC_INDEX(bmi,MAXBITMAPS)
	  && read_ok(filename)
	  && (fp = fopen(filename,"r")) != (FILE*)0
	  && (b = bitmapread(fp))) 
      {
        if (W(bitmaps[bmi])) bit_destroy(W(bitmaps[bmi]));
        W(bitmaps[bmi]) = b;
    ...
    case T_SMAP:
    {
      ...
      switch(cnt)
      {
        ...
        if (num == 0) b = screen;
        else if (ESC_INDEX1(num,MAXBITMAPS)) b = W(bitmaps[num-1]);
        break;
        case 0:			/* my window */
        free_b++;
