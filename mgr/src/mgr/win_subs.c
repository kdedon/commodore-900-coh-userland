...
void win_rop(win, window)
WINDOW *win;
BITMAP *window;
{
  register int *p = W(esc);
  register int op;
  int sx, sy, sw, sh;		/* scaled dest rect, one Scale* per statement */
  int ssx, ssy;			/* scaled source point */
  ...
  switch (W(esc_cnt)) 
  {
    ...
    sw = Scalex(p[2]);
    sh = Scaley(p[3]);
    bit_blit(window,sx,sy,sw,sh,op,(DATA*)0,0,0);
    if (Do_clip()) Set_clip(sx,sy,sx+sw,sy+sh)
    break;
    /*}}}  */
    /*{{{  4 -- ras_write  specify dest*/
    case 4:
    if (p[4]!=0 && !ESC_INDEX1(p[4],MAXBITMAPS)) break;
    sx = Scalex(p[0]);
    sy = Scaley(p[1]);
    sw = Scalex(p[2]);
    sh = Scaley(p[3]);
    ...
    if (Do_clip() && p[4]==0) Set_clip(sx,sy,sx+sw,sy+sh);
    break;
    /*}}}  */
    /*{{{  5 -- ras_copy*/
    case 5:
    ...
    ssx = Scalex(p[4]);
    ssy = Scaley(p[5]);
    bit_blit(window,sx,sy,sw,sh,op,window,ssx,ssy);
    if (Do_clip()) Set_clip(sx,sy,sx+sw,sy+sh);
    break;
    /*}}}  */
    /*{{{  7 -- ras_copy specify dst,src*/
    case 7:
    if ((p[6]!=0 && !ESC_INDEX1(p[6],MAXBITMAPS)) ||
        (p[7]!=0 && !ESC_INDEX1(p[7],MAXBITMAPS))) break;
    sx = Scalex(p[0]);
    sy = Scaley(p[1]);
    sw = Scalex(p[2]);
    sh = Scaley(p[3]);
    ...
    if (p[6]>0 && W(bitmaps)[p[6]-1]==(BITMAP*)0)
    {
      ...
      W(bitmaps)[p[6]-1] = bit_alloc(sx+sw,sy+sh,NULL_DATA,depth);
    }
    ...
    if (Do_clip() && p[6]==0) Set_clip(sx,sy,sx+sw,sy+sh);
    /*}}}  */
  }
}
/*}}}  */
/*{{{  win_map -- down load a bit map  - parse escape sequence*/
void win_map(win, window)
WINDOW *win;
BITMAP *window;
{
  /*{{{  variables*/
  ...
  int *p = W(esc);
  int op=W(op);
  int gx, gy;			/* scaled graphics point, one Scale* per statement */
  /*}}}  */
  ...
  switch(cnt)
  {
    ...
    gx = Scalex(W(gx));
    gy = Scaley(W(gy));
    bit_blit(window,gx,gy,p[0],p[1],op,W(bitmap),0,0);
    if (Do_clip())
    {
      Set_clip(gx,gy,gx+p[0],gy+p[1]);
    }
    break;
    /*}}}  */
    /*{{{  3 -- bitmap to graphics point specify dest*/
    case 3:
    if (p[2]!=0 && !ESC_INDEX1(p[2],MAXBITMAPS)) break;
    if (p[2]>0 && W(bitmaps)[p[2]-1]==(BITMAP*)0)
    {
      ...
      gx = Scalex(W(gx));
      gy = Scaley(W(gy));
      bit_blit(p[2]?W(bitmaps)[p[2]-1]:window,gx,gy,p[0],p[1],op,W(bitmap),0,0);
    }
    break;
    /*}}}  */
...
BITMAP *window;
{
  ...
  switch (W(esc_cnt)) 
  {
    ...
    /*{{{  4*/
    case 4:
    if (p[4]==0 || (ESC_INDEX1(p[4],MAXBITMAPS) && W(bitmaps)[p[4]-1]))
...
/*}}}  */
/*{{{  Bit_line*/
void Bit_line(win, dst, x1, y1, x2, y2, op)
WINDOW *win;
BITMAP *dst;
int x1;
int y1;
...
BITMAP *window;
{
  ...
  switch (W(esc_cnt)) 
  {
    ...
    case 4:
    if (ESC_INDEX1(p[4],MAXBITMAPS) && W(bitmaps)[p[4]-1])
    ...
    case 6:
    if (ESC_INDEX1(p[6],MAXBITMAPS) && W(bitmaps)[p[6]-1])
