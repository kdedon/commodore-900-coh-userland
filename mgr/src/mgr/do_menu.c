...
int exit_code;			/* valid exit codes */
   ...
   /* menu numbers reach here from the client (the menu bound to a button, or
      a link followed below), so bound the subscript and require the menu to
      exist before menu_choice() dereferences it: a client can clear a menu
      after binding it to a button. */
   if (!ESC_INDEX(menu,MAXMENU) ||
       (state = menu_list[menu]) == (struct menu_state *) 0)
      return(NULL);
   ...
   dbgprintf('m',(stderr,"  Setting up menu %d at %d,%d: valid states %s\n",
   ...
   while (!done)
     ...
     dbgprintf('m',(stderr,"  from user ...")); fflush(stderr);
     ...
     dbgprintf('m',(stderr,"got menu %d (at %d,%d) selection %d (%s)\n",
     ...
     switch (menu_exit(state)) {
        ...
        case EXIT_RIGHT:	/* slid off top the right */
             if (ESC_INDEX((next = menu_next(state)),MAXMENU) && /* link ok */
   ...
   dbgprintf('m',(stderr,"  Tearing down %d at %d,%d choice: %d, returning: %s\n",
		 menu,x,y,menu_choice(state),print_menu(menu_exit(state))));
