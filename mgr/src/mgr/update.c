...
static rectangle *got_int(); /* finds intersecting rectangle */
static void in_mouseoff();
static int in_win();
static int cmp();	/* compare for qsort */

/*}}}  */

/*{{{  gen_list -- generate a clip list*/
static int gen_list(window)
WINDOW *window;
{
...
/* look at rect list	DEBUG code, commented out!
	...
	for(list=(struct rect_list *) W(clip_list);list;list = list->next) {
		...
		bit_blit(W(border), x, y, wide, high, BIT_NOT(BIT_DST),(BITMAP *)0,0,0);
...
static void do_update(win, clipp)
WINDOW *win;
rect *clipp;
{
...
static rectangle *got_int(r1, r2, bwid)
rectangle *r1;
/* rect 1 */
			  rect *r2;
/* other rect (should both be same struct) */
			  int bwid;
{
