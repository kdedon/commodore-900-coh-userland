...
register WINDOW *check;			/* window to check covering against */
   ...
   for(win=active;win != (WINDOW *) 0;win=win->next)
       if (win!=check && intersect(win,check) && W(flags)&W_ACTIVE) {
          if (!save_win(win)) {
             /* Leave it active: without a save bitmap there is nowhere for
                its output to go, and the covering window will overdraw it
...
void un_covered()
   ...
   for(win=active;win != (WINDOW *) 0;win=W(next)) {
      ...
      if (cover && W(flags)&W_ACTIVE && W(save) != (BITMAP *) 0) {
          do_event(EVENT_COVERED,win,E_MAIN);
          W(flags) &= ~W_ACTIVE;
          if (!(W(flags)&W_LOOK))
             FD_CLR( W(to_fd), &mask);
...
/*	Returns 1 when the window's contents are held in W(save), 0 when there
	is no room for them.  A window's save bitmap is the largest single
	allocation the server makes -- an 80x24 window in a 9x16 font is 36 KB
	-- so it is the one that a caller must expect to be refused.  A window
	that is not W_ACTIVE is drawn through W(save) (put_window(), border()),
	so a caller may only deactivate a window whose save succeeded.
*/
int save_win(win)
WINDOW *win;
{
   ...
   if (W(save) == (BITMAP *) 0) {
      dbgprintf('o',(stderr,"no save space for %s\r\n",W(tty)));
      return(0);
      }
	...
	switch(W(curs_type)) {
		case CS_BLOCK:
			bit_blit(W(window), W(x)+W(text.x),
				...
				PUTOP(BIT_NOT(BIT_DST),W(style)), (BITMAP *)0, 0, 0);
			break;
		case CS_BOX:
			bit_blit(W(window), W(x)+W(text.x),
