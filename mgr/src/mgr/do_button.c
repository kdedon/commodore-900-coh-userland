...
void quit()
{
   ...
   state = menu_define(font,quit_menu,(char **)0,0,MENU_COLOR);
      ...
      case BUTTON_SYS:				/* for system operation */
           ...
           if (win == active || win == (WINDOW *) 0) {
              if (active && win == active) {
                 state = menu_define(font,active_menu,(char **)0,0,MENU_COLOR);
                 which_menu = 1;
                 }
              else if (next_window >= MAXWIN) {
                 state = menu_define(font,full_menu,(char **)0,0,MENU_COLOR);
                 which_menu = 2;
                 }
              else {
                 state = menu_define(font,main_menu,(char **)0,0,MENU_COLOR);
                 which_menu = 3;
                 }
              if (active) {
                 cursor_off();
