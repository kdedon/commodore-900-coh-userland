...
next_windowset_id()
   {
	 ...
	 *cp = 0;
      ...
      /*	A window's set ID is not necessarily one this function issued:
		a `windowsetid' line in the startup file assigns one directly,
		so the index is bounded here rather than trusted.
      */
      for( win = active;  win != (WINDOW *)0;  win = W(next) )
	 if( W(setid) > 0  &&  W(setid) <= MAXWIN )
	    list[ W(setid) ] = 1;

      /*	There is no window set ID zero.  list[MAXWIN+1] is never set --
		nothing above MAXWIN is marked -- and that is what stops this
		scan, which has no bound of its own.
      */
      for( cp = list + 1;  *cp;  cp++ )
	 ;
...
void new_window()
   ...
   (void) create_window( mousex, mousey, dx, dy, -1, (char **)0 );
   }
/*}}}  */
