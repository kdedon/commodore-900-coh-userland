...
#else /* BSD */
#define wait_nohang(statusp)  wait3( statusp, WNOHANG, (char *)0)
#endif
/*}}}  */
...
#ifdef C900
      /* W_DIED says the window's pty hung up, and this is the only reaper on
         a system with no SIGCHLD, so the wait happens either way. */
      if (W(pid)>1) {
#else
