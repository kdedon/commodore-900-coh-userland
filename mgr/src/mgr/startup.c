...
/*}}}  */
/*{{{  free_shell*/
/*	Release every element of a window's shell vector and empty the vector.
	One vector serves every `window' in the startup file in turn, so an
	element left in it belongs to the next window that names no `shell' of
	its own -- or to the ALT-n template, which is captured from the same
	vector.
*/
static void free_shell( shell )
char	*shell[];
{
	register int	i;
...
/*	Returns 0 when a window was made and `active' now addresses it, -1 when
	create_window() made none.  A caller that goes on to touch ACTIVE(...)
	must check: with no window ever made `active' is null, and with windows
	already open it still addresses the one that was active before, so an
	unchecked ACTIVE(setid) = ... writes into a live window.
*/
static int
...
unsigned char *init, *start;
{
    ...
    i = create_window(x, y, dx, dy, font, *shell?shell:(char **)0);
    if( mouse_was_on)  MOUSE_ON(screen,mousex,mousey);

    /*	Everything below addresses the window that was just made, through
	`active'.  create_window() leaves `active' alone when it makes none --
	null if this would have been the first window -- so there is nothing
	here to address.
    */
    if( i < 0 )
       return(-1);
...
void startup(name) char *name;
   ...
   while (fgets(line,sizeof(line),file) != NULL) {
      ...
      /* got a window */
         ...
         if (gotwindow==1) {
	    /*	A window whose geometry check_window() refuses costs itself and
		nothing else: this line is also the `window' line that declares
		the NEXT one, so the rejection discards the pending window's
		state and then goes on to read the new declaration below.
	    */
	    ...
	    if( !active ) {
		 ...
		 log_time();
#endif
       if( dowindow( x, y, dx, dy, curr_font, shell, flags, init, start ) == 0 ) {
	 ...
	 /* next_windowset_id() indexes a MAXWIN table by set ID. */
	 windowsetid = atoi( fields[1] );
	 if( windowsetid < 1  ||  windowsetid > MAXWIN )
	    windowsetid = 0;
      }
...
void initwindow()
{
    (void) dowindow( newx, newy, newdx, newdy, newfont, newshell, newflags,
	newinit, newstart );
}
/*}}}  */
