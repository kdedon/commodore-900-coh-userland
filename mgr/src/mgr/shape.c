...
int dy;
{
  ...
  /* The old contents come back from W(save); save_win() supplies none when
     there is no room for one, and then the reshaped window starts empty. */
  if (ACTIVE(save) != (BITMAP *) 0)
  ...
  if (ACTIVE(y) > BIT_HIGH(ACTIVE(window))) 
  {
    ...
    scroll(ACTIVE(window),BIT_NULL,BIT_HIGH(ACTIVE(window)),
