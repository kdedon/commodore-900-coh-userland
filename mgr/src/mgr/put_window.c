...
int put_window(win, buff, buff_count)
WINDOW *win;
unsigned char *buff;
int buff_count;
{
  ...
  /*{{{  set up environment*/
  /* An inactive window's output goes into its saved image, not onto the
     screen.  bit_create() reads its source bitmap, so a window with no saved
     image is left on the screen rather than written through a null one. */
  if (W(flags)&W_ACTIVE || W(save)==(BITMAP*)0) window=W(window);
  else
  {
  ...
  switch ((int)(W(flags)&W_STATE))	/* flags is long; W_STATE is 0x402 */
  {
    ...
    case W_ESCAPE:
      ...
      switch(c)
      {
        ...
        case '5': case '6': case '7': case '8': case '9':
	      ...
	      /* Saturate rather than wrap: int is 16 bits here, so a client
	         can send more digits than a parameter holds, and a wrapped
	         large positive arrives negative -- indistinguishable from an
	         index that reaches back into the window structure. */
	      if(n >= 0)
		 n = n > (ESC_PARAM_MAX-d)/10 ? ESC_PARAM_MAX : n*10 + d;
	      else if(n == ESC_PARAM_MIN)
		 ;			/* already saturated */
	      else
		 n = -n > (ESC_PARAM_MAX-d)/10 ? ESC_PARAM_MIN : n*10 - d;
              W(flags) |= W_ESCAPE;
        ...
        case E_CLEAREOL:               /* clear to end of line */
              bit_blit(text,W(x),W(y)-fsizehigh,T_WIDE-W(x),fsizehigh,BG_OP,(BITMAP *)0,0,0);
              if (Do_clip())
        ...
        case E_CLEAREOS:               /* clear to end of window */
              bit_blit(text,W(x),W(y)-fsizehigh,T_WIDE-W(x),fsizehigh,BG_OP,(BITMAP *)0,0,0);
              bit_blit(text,0,W(y),T_WIDE,T_HIGH-W(y),BG_OP,(BITMAP *)0,0,0);
              if (Do_clip())
        ...
        case E_BLEEP:
              if (cnt>2) {
                 ...
                 bit_blit(screen,p[0],p[1],p[2],p[3],BIT_NOT(BIT_DST),(BITMAP *)0,0,0);
                 fsleep();
                 bit_blit(screen,p[0],p[1],p[2],p[3],BIT_NOT(BIT_DST),(BITMAP *)0,0,0);
                 done++;
                 }
        ...
        case E_MOUSE:
		 ...
		 if (cnt == 0) {
		    ...
		    if (ESC_INDEX1(bn,MAXBITMAPS)
			       && cursor_ok(W(bitmaps)[bn-1])
			       && (W(cursor)=bit_alloc(16,32,NULL_DATA,1)) != NULL) {
        ...
        case E_XMENU:
              ...
              switch(cnt) {
                 case 0:                       /* remove menu from display */
                    if (ESC_INDEX(p[0],MAXMENU) && (menu=W(menus[p[0]])))
                       menu_remove(menu);
        ...
        case E_MENU:                   /* get a menu */
              ...
              /* The sign of the number is the button, not part of the
                 number; the magnitude is an index into menus[MAXMENU] and is
                 bounded at each use below.  m_clearmenu() deliberately sends
                 an out of range number (999) to mean "no menu on this
                 button", so an out of range number must reach the cnt==0
                 clearing path rather than be rejected here. */
              ...
              if (cnt > 2) {
                 ...
                 /* child is a link value: m_unlinkmenu() sends -1 for
                    "no link", and the >= 0 test is made where the link is
                    followed (do_menus()). */
                 if (!ESC_INDEX(parent,MAXMENU) || child >= MAXMENU ||
              ...
              else if (ESC_INDEX(n,MAXMENU) && W(menus)[n]) {
        ...
        case E_BITGET:
        {
          ...
          /* offset and size are client supplied and signed: a negative
             offset reads before the bitmap data and a negative size becomes
             a huge count in write().  Test the sum by subtraction so it
             cannot overflow 16 bits either. */
          if
          (
            cnt>1
            && ESC_INDEX1(which,MAXBITMAPS)
            && (m=W(bitmaps)[which-1])!=(BITMAP*)0
            && offset>=0 && size>=0
            && offset<=B_SIZE8(BIT_WIDE(m),BIT_HIGH(m),BIT_DEPTH(m))
            && size<=B_SIZE8(BIT_WIDE(m),BIT_HIGH(m),BIT_DEPTH(m))-offset
          )
          {
            data=bit_save(m);
        ...
        case E_BITVALUE:
             ...
             if ((cnt==2||cnt==3) && ESC_INDEX1(m,MAXBITMAPS)) {
		 map = W(bitmaps)[m-1];
		 if (map) {
	      ...
	      int bmnbr = W(esc)[0];
              ...
              switch(cnt) {
                 case 0:               /* destroy a bitmap */
                    if (ESC_INDEX1(bmnbr,MAXBITMAPS) && W(bitmaps)[bmnbr-1]) {
        ...
        if (cnt>=4)
        {
           /* the download length is client supplied and signed: a negative
              length becomes a huge malloc() argument, and a zero length a
              buffer that put_window() still writes one byte into */
           if (W(esc[W(esc_cnt)])>0 && (W(snarf)=malloc(W(esc[W(esc_cnt)])))!=0)
           {
              W(esc)[TEXT_COUNT]=0;
              W(code)=T_BITMAP;
              W(flags)|=W_TEXT;
