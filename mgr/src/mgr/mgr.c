...
#else
#ifdef WUNTRACED	/* BSD */
  pid = wait3(&status,WUNTRACED,(char *)NULL);
  someonedied = pid > 0 && !WIFSTOPPED(status);
#else			/* other Unix */
  pid = wait(&status);
  someonedied = pid > 0;
...
#     endif
   ...
   /*}}}  */
   /*{{{  open the mouse*/
   /* A failure here is not fatal: with mouse < 0 the descriptor is kept out
    * of the select() mask and mgr runs from the keyboard alone.  It is not
    * left at 0 either -- that is the keyboard, and every keystroke would then
    * be read as pointer data. */
   if ((mouse=open(mouse_dev,O_RDWR)) < 0)
      fprintf(stderr,"mgr: no mouse on %s: %s\r\n",mouse_dev,strerror(errno));
   mfd=mouse;
   /*}}}  */
   /*{{{  find screen*/
   ...
   /* After the screen, because the pointer is clamped to its dimensions. */
   if (mouse >= 0)
      ms_init(BIT_WIDE(screen),BIT_HIGH(screen),mouse_type);
   mousex=mousey=32;
   ...
   /*}}}  */
   copyright(screen, "");
   mouse_save=bit_alloc(16,32,NULL_DATA,BIT_DEPTH(screen));
...
#   endif
   ...
   while(1) {
	 ...
	 poll_time = NULL;
      if (select(FD_SETSIZE,&reads,(fd_set *)0,(fd_set *)0,poll_time) <0) {
#ifdef DEBUG
      ...
      if (mouse >= 0 && FD_ISSET( mouse, &reads)) {
	 do {
	    proc_mouse(mouse);
	 } while(mouse_count() > 0);
      }
...
#ifdef C900
               /* No SIGCHLD on this machine, so the pty is what says the
                  program in the window is gone: the master reads EIO once the
                  last slave has closed (sys/drv/pty.c, p_mopen == 3), and
                  polls POLLHUP, which is why the select above returned it.  A
                  read of 0 is the same news from any other kind of descriptor.
                  The other way a nodelay master read fails is EAGAIN, which
                  says only that there is no output yet. */
               if (W(max) == 0 || errno == EIO)
                  W(flags) |= W_DIED;
#endif
#ifdef KILL
