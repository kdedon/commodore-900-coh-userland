...
void
ms_init(screen_w, screen_h, mouse_type)
int screen_w;
int screen_h;
char *mouse_type;
{
...
#ifdef C900
	else if (strcasecmp(mouse_type,"c900")==0)           mtype = P_C900;
#endif
    }

    init = 1;
    if (mtype != P_BM && mtype != P_PS2 && mtype != P_C900)
    {
...
{
    unsigned char buf[5];
    int dx, dy;			/* wider than a packet byte: a native event
				 * is not limited to +-127 on an axis */
    int i, acc;
#ifdef C900
    struct msevent nev;
#endif

    if (init == 0) ms_init(mx,my,(char *)0);
    if (mfd == -1)
	return -1;
    ...
    if (mtype == P_C900)
    {
	/* One whole event per read: the device transfers nothing smaller,
	 * so there is no header to scan for and no framing to recover. */
	if (read(mfd, (char *)&nev, sizeof nev) != sizeof nev)
	    return -1;

	ev->ev_butstate = 0;
	if (nev.ms_buttons & MSBLEFT)
	    ev->ev_butstate |= MS_BUTLEFT;
	if (nev.ms_buttons & MSBMIDDLE)
	    ev->ev_butstate |= MS_BUTMIDDLE;
	if (nev.ms_buttons & MSBRIGHT)
	    ev->ev_butstate |= MS_BUTRIGHT;

	/* Right and down positive, matching the sense the serial protocols
	 * are decoded into above. */
	dx = nev.ms_dx;
	dy = nev.ms_dy;
	goto have_event;
    }
