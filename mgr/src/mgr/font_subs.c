...
struct font *open_font(file) char *file;
{
  ...
  bytes = bit_size(BIT_WIDE(font->data),BIT_HIGH(font->data),BIT_DEPTH(font->data));
  fread(BIT_DATA(font->data), (unsigned int)bytes, 1, fp);
  SET_DIRTY(font->data);
  /* create individual characters */
...
int enhance_font(font)struct font *font;                    /* font to be enhanced */
{
  ...
  /* make data structures larger, copy existing data.  Both allocations are
     made before anything is torn down, so a font that cannot be enhanced --
     the enhanced bitmap is four times as wide, and a wide font's can pass
     what one malloc will hold -- is left exactly as it was. */
