...
#ifndef HAVE_PTMX	/* no /dev/ptmx, grantpt, unlockpt or ptsname here */
   line[5] = 'p';
...
int get_command(argv,file) char **argv; int *file;
{
   ...
   if (argv == (char **) 0 )
   {
      /* A LOGIN shell (argv[0] begins with `-'), so that /etc/profile and the
         user's own .profile are read in every window: PATH, TERM and MAIL are
         theirs to set.  A window opened for a named command is not a shell and
         is left alone. */
      char *base;

      argv = arg;
      name = get_path(shell?shell:SHELL);
      if (name == (char *) 0 || *name == '\0')
         return(-2);
      base = strrchr(name,'/');
      dash[0] = '-';
      strncpy(dash+1,base?base+1:name,sizeof(dash)-2);
