...
static char rcsid[] = "$Id: tcap.c,v 4.10 1993/10/04 16:59:45 mikes Exp $";
#endif
/*
...
tcapopen()
{
    ...
    CE = tgetstr("ce", &p);
    UP = tgetstr("up", &p);
    BC = tgetstr("bc", &p);		/* tgoto() reads it for a left move */
    SE = tgetstr("se", &p);
    SO = tgetstr("so", &p);
    DL = tgetstr("dl", &p);
    AL = tgetstr("al", &p);
    ...
    if(TI && !Pmaster) {
	putpad(TI);			/* any initialization termcap requires */
	if (CS != NULL)
