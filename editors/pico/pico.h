/*
 * $Id: pico.h,v 4.4 1993/09/24 17:13:05 mikes Exp $
 *
...
typedef struct pico_struct {
    char  *msgtext;			/* ptrs to malloc'd arrays of char */
...
#define	P_REPLY		0x4000		/* start composer in body	 */
...
char	*version = "2.1";		/* PICO version number */
#else
...
char *pico_get(void);
void  pico_give(char *);
int   pico_readc(char *, unsigned char *);
int   pico_writec(char *, int);
int   pico_puts(char *, char *);
int   pico_seek(char *, long, int);
int   pico_replace(char *, char *);
#else
int   pico();
char *pico_get();
void  pico_give();
