...
static char rcsid[] = "$Id: composer.c,v 4.6 1993/09/30 22:02:10 mikes Exp $";
#endif
/*
...
int f, n;
{
	...
	ch = LineEdit(TRUE);			/* work on the current line */
	  ...
	  case (CTRL|'X') :			/* Done. Send it. */
		...
		if(SyncAttach() < 0){
		    if(mlyesno("Problem with attachments. Send anyway?",
			       FALSE) != TRUE){
	  ...
	  default :				/* huh? */
	    if(ch&CTRL)
	      emlwrite("\007Unknown command: ^%c", (char *)(ch&0xff));
	    else
	  case BADESC:
...
int	allowedit;
{
	...
	if(ch == NODATA || time_to_check()){	/* new mail ? */
		...
		if(ods.cur_e == FCCHDR && !fallowc((char)ch)){
		    ...
		    emlwrite("\007Can't have a '%c' in folder name",(char *)ch);
		    continue;
		}
