/*
 * $Id: os_unix.h,v 4.16 1993/10/06 17:14:33 mikes Exp $
 *
...
#endif
#if	defined(COHERENT)
/* <dirent.h> declares the directory-stream calls only for the 386; the
 * directory reader in cohdir.c supplies them here, and an undeclared
 * readdir() would come back as a 16-bit int and lose half a far pointer. */
extern DIR		opendir();
...
#if	defined(COHERENT)
#undef	QcompType
#define QcompType char			/* this compiler has no `void *' */
#endif
...
/* The two bcopy() calls move one screen row to another, never within a row,
 * so the non-overlapping memcpy() this libc has is enough. */
#if	defined(COHERENT)
#define bcopy(a,b,s) memcpy (b, a, s)
#endif
