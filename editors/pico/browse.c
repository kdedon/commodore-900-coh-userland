...
static char rcsid[] = "$Id: browse.c,v 4.8 1993/09/27 16:31:28 mikes Exp $";
#endif
/*
...
char *dir, *fn, *sz;			/* dir, name and optional size */
{
	...
	switch(normal(c, bfmappings, 2)){	/* process commands */
	  ...
	  case 'X':
	    if(!(gmode&MDBRONLY)){
		emlwrite("\007Unknown command '%c'", (char *)c);
		break;
	    }
...
/* remove break and sleep after help text is installed */
	  ...
	  default:				/* what? */
	    if(c < 0xff)
	      ...
	      emlwrite("\007Unknown command: ^%c", (char *)(c&0xff));
	    else
...
char *dir;
{
    ...
    if(Pmaster)
      ...
      sprintf(s,"   PICO %s", (gmode&MDBRONLY) ? "BROWSER" : version);
