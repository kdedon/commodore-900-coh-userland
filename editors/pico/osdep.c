...
static char rcsid[] = "$Id: os_unix.c,v 4.26 1993/10/06 17:12:54 mikes Exp $";
#endif
/*
...
static  jmp_buf	alrm_state;			/* stack env for SIGALRM */
...
ttopen()
{
    ...
    signal(SIGALRM, do_alarm_signal); /* timer for new mail */
#ifdef	SIGTSTP
...
do_alarm_signal()
{
    signal(SIGALRM, do_alarm_signal);
...
int  *n;
{
    ...
    *n = 0;
    ...
    /* malloc() takes an unsigned int, which is 16 bits here, so the
     * directory's size has to be narrowed to one before it is passed and
     * checked against what that width can hold. */
    if(l > (long)60000){
	emlwrite("\007Directory too large to list", NULL);
	return(NULL);
    }
    if((names=(char *)malloc((unsigned)l)) == NULL){
...
#if	defined(sv3) || defined(ct) || defined(COHERENT)
