...
/*
 * tgoto() reads UP and BC to build a relative motion, and tputs() writes
 * PC as the pad character; libterm declares all three extern and leaves the
 * definitions to the program that links against it.
 */
char	*UP;
char	*BC;
char	PC;
...
tcapopen()
{
	 ...
	 */
	{
		...
		UP = tgetstr("up", &ptr); /* cursor up one line */
		BC = tgetstr("bc", &ptr); /* backspace, if not ^H */
