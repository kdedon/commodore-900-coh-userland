...
void nc_draw_sprites();		/* defined below; called by nc_draw_editor */
 ...
 */
static chtype
zone_glyph(t, letter, fg, bg, baseAttr, emptyBase, builtBase, builtHi, dmod, houseLo, houseHi)
int t;
int letter;
int fg;
int bg;
...
static void
demand_row(y, x, lbl, valve, color, w)
int y;
int x;
char lbl;
int valve;
...
int w;
{
  ...
  addch('[');
  if (Gfx->emojiui) {
    full = (v >= 0) ? "\342\226\210" : "\342\226\222";
    for (k = 0; k < 6; k++) addstr(k < n ? full : "\342\226\221");
  } else {
...
SimView *view;
{
   ...
   * then the city stats, stacked vertically.  On short terminals the tail
   * is clipped, never the grid. */
  iy = top + ngr + 2;
...
char *path;
{
  ...
  chtype c;
  char cpath[512];
  static char digit[10] = "krgybmcw ";	/* curses bg color -> letter */
