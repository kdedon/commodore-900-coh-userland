...
static ToolBtn toolbar[] = {
  /* row 0: zones            */ { residentialState,'R', "R",  "RE", "\360\237\217\240", COLOR_BLACK,  COLOR_CYAN    },
                                { commercialState, 'C', "C",  "CO", "\360\237\217\254", COLOR_WHITE,  COLOR_BLUE    },
                                { industrialState, 'I', "I",  "IN", "\360\237\217\255", COLOR_YELLOW, COLOR_MAGENTA },
  /* row 1: services + query */ { fireState,       'F', "F",  "FI", "\360\237\232\222", COLOR_WHITE,  COLOR_RED     },
                                { queryState,      'Q', "?",  "QY", "\360\237\224\215", COLOR_BLACK,  COLOR_WHITE   },
                                { policeState,     'P', "P",  "PD", "\360\237\221\256", COLOR_WHITE,  COLOR_RED     },
  /* row 2: utilities        */ { wireState,       'W', "W",  "WI", "\342\232\241",     COLOR_RED,    COLOR_WHITE   },
                                { dozeState,       'B', "B",  "BU", "\360\237\232\234", COLOR_BLACK,  COLOR_YELLOW  },
                                { parkState,       'K', "\"", "PK", "\360\237\214\263", COLOR_BLACK,  COLOR_GREEN   },
  /* row 3: transport        */ { rrState,         'L', "RR", "RR", "\360\237\232\202", COLOR_BLACK,  COLOR_WHITE   },
                                { roadState,       'O', "RO", "RO", "\360\237\232\227", COLOR_WHITE,  COLOR_BLACK   },
  /* row 4: big buildings    */ { stadiumState,    'S', "ST", "ST", "\342\232\275",     COLOR_BLACK,  COLOR_WHITE   },
                                { seaportState,    'E', "PO", "PO", "\342\232\223",     COLOR_WHITE,  COLOR_BLUE    },
  /* row 5: power plants      */ { powerState,     'G', "CP", "CP", "\360\237\224\214", COLOR_WHITE,  COLOR_RED     },
                                { nuclearState,    'N', "NP", "NP", "\342\230\242 ",    COLOR_BLACK,  COLOR_GREEN   },
  /* row 6: airport           */ { airportState,   'A', "AP", "AP", "\360\237\233\253", COLOR_BLACK,  COLOR_WHITE   }
};
...
static short tool_cycle_state(i)
int i;
{ return toolbar[i].state; }

/* toolbar accessors for the renderer */
int    nc_toolbar_count()
{ return NTOOLS; }
int    nc_toolbar_state(i)
int i;
{ return toolbar[i].state; }
int    nc_toolbar_color(i)
int i;
{ return toolbar[i].fg; }
int    nc_toolbar_bg(i)
int i;
{ return toolbar[i].bg; }
char   nc_toolbar_key(i)
int i;
{ return toolbar[i].key; }
char  *nc_toolbar_code(i)
int i;
{ return toolbar[i].code; }
char  *nc_toolbar_lcode(i)
int i;
{ return toolbar[i].lcode; }
char  *nc_toolbar_emoji(i)
int i;
{ return toolbar[i].emoji; }
int    nc_toolbar_gridrows()
{ return NGRIDROWS; }
int    nc_toolbar_rowlen(r)
int r;
{ return grid_cols[r]; }
...
void
nc_show_zone_status(zone, s0, s1, s2, s3, s4, x, y)
char *zone;
char *s0;
char *s1;
char *s2;
