...
char *dir;
{
  ...
  /* Accept the three city sizes.  A chain, not a switch: `size' is a QUAD and
   * two of the three constants are outside a 16-bit int, so neither the switch
   * control nor its case labels can be int on this target. */
  if ((size != 27120L) &&		/* normal city */
      (size != 99120L) &&		/* 2x2 city    */
      (size != 219120L))		/* 3x3 city    */
    return (0);
...
static int _load_mem();	/* defined below */
...
char *filename;
{
  ...
  l = (long)(policePercent * 65536.0);
  HALF_SWAP_LONGS(&l, 1);
  (*(QUAD *)(MiscHis + 58)) = l;

  l = (long)(firePercent * 65536.0);
  HALF_SWAP_LONGS(&l, 1);
  (*(QUAD *)(MiscHis + 60)) = l;

  l = (long)(roadPercent * 65536.0);
  HALF_SWAP_LONGS(&l, 1);
  (*(QUAD *)(MiscHis + 62)) = l;
 ...
 * baked-in scenarios (snro.NNN) and example cities (cities/<name>.cty).  Reads the
...
QUAD size;
{
  ...
  if ((size != 27120L) &&	/* normal city -- see _load_file on the chain */
      (size != 99120L) &&	/* 2x2 city    */
      (size != 219120L))	/* 3x3 city    */
    return 0;
...
/* Load one of the baked-in example cities (cities/<name>.cty) by basename, e.g.
