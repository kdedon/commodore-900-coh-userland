...
int SetValves()
{
  ...
  double Employment, Migration, Births, LaborBase, IntMarket;
  double Rratio, Cratio, Iratio, temp;
  double NormResPop, PjResPop, PjComPop, PjIndPop;
  register short z;
...
int CollectTax()
{
  static double RLevels[3] = { 0.7, 0.9, 1.2 };
  static double FLevels[3] = { 1.4, 1.2, 0.8 };
  short z;
...
int UpdateFundEffects()
{
  if (RoadFund)
    RoadEffect = (short)(((double)RoadSpend /
			  (double)RoadFund) * 32.0);
  else
    RoadEffect = 32;

  if (PoliceFund)
    PoliceEffect = (short)(((double)PoliceSpend /
			    (double)PoliceFund) * 1000.0);
  else
    PoliceEffect = 1000;

  if (FireFund)
    FireEffect = (short)(((double)FireSpend /
			  (double)FireFund) * 1000.0);
  else
    FireEffect = 1000;
...
#define RANDOM_RANGE 0xffff		/* Rand16()'s inclusive maximum */

/* A value in 0..range inclusive, drawn without modulo bias: the largest
 * multiple of range that Rand16() can reach is the acceptance bound, and
 * anything at or above it is redrawn.  All of it is unsigned, because
 * Rand16() spans the whole 16-bit range and both the bound and the draw are
 * negative numbers when read as int. */
short
Rand(range)
short range;
{
  unsigned int maxMultiple, rnum;

  range++;
  maxMultiple = (unsigned int)RANDOM_RANGE / (unsigned int)range;
  maxMultiple *= (unsigned int)range;
  while ((rnum = (unsigned int)Rand16()) >= maxMultiple)
    continue;
  return ((short)(rnum % (unsigned int)range));
}
...
/* Reflect the upper half of Rand16()'s range onto the negative integers, so
 * the result is signed and uniform.  The subtraction is unsigned and wraps;
 * the int conversion of the wrapped value is the negative wanted. */
int
Rand16Signed()
{
  unsigned int u = (unsigned int)Rand16();
...
int RandomlySeedRand()
{
  ...
  /* The xor is a long; SeedRand takes an int and there are no prototypes, so
   * the narrowing has to be written or the seed is taken from the high half. */
  SeedRand((int)(time.tv_usec ^ time.tv_sec ^ (long)sim_rand()));
}
