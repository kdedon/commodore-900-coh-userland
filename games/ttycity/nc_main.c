...
#include "sim.h"
#include <curses.h>
/* no <locale.h>: c900.h supplies setlocale()/LC_* as no-ops */
#include "nc.h"
...
char *argv[];
{
  ...
  /* Terrain generation runs for a long time on a slow machine and draws
   * nothing, so the screen would otherwise stay exactly as the shell left
   * it until the first full frame.  Clear it and say what is happening. */
  erase();
  mvaddstr(0, 0, "ttycity: generating terrain, please wait ...");
  refresh();
  ...
  while (!Quitting && !tkMustExit) {
    ...
    nc_draw_editor(EditorView);
    nc_draw_status(EditorView);
    nc_minimap_late();	/* narrow-terminal overlay, over the editor */
