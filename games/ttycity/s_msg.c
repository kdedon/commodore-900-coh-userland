...
int DoAutoGoto(x, y, msg)
short x;
short y;
char *msg;
{
  SetMessageField(msg);
  nc_auto_goto(x, y);		/* ncurses port: pan the editor cursor there */
}


int SetMessageField(str)
char *str;
{
  char buf[256];
...
int DoShowPicture(id)
short id;
{
  nc_show_notice(id);		/* ncurses port: popup notice (text from stri.301) */
}
