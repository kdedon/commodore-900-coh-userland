...
int doScoreCard()
{
  ...
  sprintf(pop, "%ld", (long)CityPop);
  sprintf(delta, "%ld", (long)deltaCityPop);
  sprintf(assessed, "%ld", (long)CityAssValue);
...
int SetEvaluation(changed, score, ps0, ps1, ps2, ps3, pv0, pv1, pv2, pv3, pop, delta, assessed_dollars, cityclass, citylevel, goodyes, goodno, title)
char *changed;
char *score;
char *ps0;
char *ps1;
