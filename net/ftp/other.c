...
char password[64];
   ...
   if(cmdargc < 2) {
	/* termio, not POSIX termios: COHERENT has no tcgetattr/tcsetattr. */
	ioctl(fileno(stdout), TCGETA, &oldtty);
	newtty = oldtty;
	newtty.c_lflag &= ~ECHO;
	ioctl(fileno(stdout), TCSETA, &newtty);
	readline("Password: ", password, sizeof(password));
	ioctl(fileno(stdout), TCSETA, &oldtty);
	printf("\n");
	pass = password;
   }
