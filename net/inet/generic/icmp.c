...
icmp_port_t *icmp_port;
{
	...
	case ICPS_BEGIN:
		...
		/* (put_pkt_t)0, not a bare 0: icmp wants no packet-mode
		 * delivery, but ip_open is K&R with no prototype, so a plain 0
		 * pushes 2 bytes (int) where the callee reads 4 (a far function
		 * pointer).  put_pkt then came out non-NULL -- high half zero,
		 * low half whatever followed the argument list -- so
		 * ip_port_arrive took the if_put_pkt branch and called a wild
		 * address, killing the daemon on the first ICMP packet it was
		 * handed.  tcp and udp pass real functions here and were fine. */
...
icmp_hdr_t *icmp_hdr;
{
		...
		bufp += entry_size)
	{
		ipr_add_oroute(icmp_port->icp_ipport, HTONL(0L), HTONL(0L), 
			*(ipaddr_t *)bufp, (time_t)lifetime * HZ, 1, 0, 
