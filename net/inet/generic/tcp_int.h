...
typedef struct tcp_port
{
	...
	int tp_ipfd;
	acc_t *tp_pack;
	/* No copy of the interface address is kept here: see udp_port_t in
	 * udp.c.  A connection still has its own tc_locaddr, because that must
	 * NOT follow a later ifconfig -- an established connection keeps the
	 * address it was opened with -- but it is taken live at open time.
	 */
