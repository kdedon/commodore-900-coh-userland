...
/*
 * ip.h is NOT inside the CRAMPED guard, and must not be put back inside it.
 * This file calls ip_get_ifaddr(), which returns a 32-bit ipaddr_t; with no
 * prototype in scope K&R gives an undeclared function a 16-bit int return, and
 * the compiler emits `LD R12,R1' + `EXTS RR0' -- the address arrives with its
 * high half thrown away and sign-extended back.  The IP header still leaves
 * with the right source (ip_send() stamps ih_src itself), so the only symptom
 * is a TCP checksum computed over an address the packet does not carry: every
 * outgoing SYN dropped by the far end, and BAD-TCP-CKSUM on the wire.  Measured
 * that way, with the guard in place.  CRAMPED is true on this target.
 */
#include "ip.h"
#if !CRAMPED
#include "io.h"
...
tcp_port_t *tcp_port;
{
	...
	case TPS_MAIN:
		...
		tcp_conn->tc_locaddr= ip_get_ifaddr(tcp_port->tp_ipdev);
...
assert (ipconf->nwic_flags & NWIC_IPADDR_SET);
			/* Flags only -- the address is read live where it is
			 * used.  See tcp_port_t in tcp_int.h.
			 */
			bf_afree(data);
		}
		break;
...
assert (conf_acc->acc_length == sizeof(*tcp_conf));
		...
		*tcp_conf= tcp_fd->tf_tcpconf;
		...
		tcp_conf->nwtc_locaddr= ip_get_ifaddr(tcp_fd->tf_port->tp_ipdev);
...
tcp_fd_t *tcp_fd;
{
	...
	tcp_conn->tc_locaddr= ip_get_ifaddr(tcp_fd->tf_port->tp_ipdev);
...
tcp_fd_t *tcp_fd;
{
	...
	/* The same hole tcp_listen() had, and closed the same way: tf_conn is
	 * overwritten below, so a descriptor that already holds a connection
	 * would leave that one behind still naming this fd.  The assert is
	...
	tcp_conn= find_conn_entry(tcp_fd->tf_tcpconf.nwtc_locport,
		ip_get_ifaddr(tcp_fd->tf_port->tp_ipdev),
...
tcp_fd_t *tcp_fd;
{
	...
	tcp_conn= tcp_fd->tf_conn;
	/*
	 * A descriptor already holding a connection must not be given a second
	 * one.  Everything below assigns tf_conn, so falling through here
	 * overwrites the pointer and leaves the first connection with tc_fd
	 * still naming this descriptor: two connections claiming one fd, one of
	 * which completes the peer's handshake while the fd reads and writes on
	 * the other.  That is a segment ACKed by the far stack and retransmitted
	 * forever, with a read that never returns.
	 *
	 * `assert(!tcp_conn)' was the only thing standing between that and the
	 * caller, and this stack is built with NDEBUG -- CRAMPED -- so it is not
	 * there.  An assertion that is compiled out is not a check, and a
	 * protocol error a client can reach must be answered rather than
	 * asserted: a second listen on a busy descriptor is EISCONN, which is
	 * what a BSD program expects and what its perror() will say.
	 *
	 * The client side is fixed too, and that is the real repair: a signal
	 * that abandons a suspended NWIOTCPLISTEN now cancels it (inet_chan.c
	 * ichan_cancel), so the descriptor is free before the retry. This is
	 * what makes reaching here impossible rather than merely harmless.
	 */
	if (tcp_conn)
	{
		tcp_fd->tf_flags &= ~TFF_IOCTL_IP;
