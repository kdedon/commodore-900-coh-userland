...
typedef struct udp_port
{
	...
	int up_ipdev;
	acc_t *up_wr_pack;
	/* No copy of the interface address is kept here.  `ifconfig' can change
	 * it after this port has started, nothing tells the port when it does,
	 * and every use below therefore reads it live with
	 * ip_get_ifaddr(up_ipdev).
	 */
...
int for_ioctl;
{
	...
	case UPS_GETCONF:
		...
		else
		{
			...
			/* The reply is read for its flags only: it says the
			 * interface now HAS an address, which is what this
			 * state was waiting for.  The address itself is not
			 * copied -- see up_ipdev in udp_port_t.
			 */
...
assert (opt_acc->acc_length == sizeof(*udp_opt));
		...
		*udp_opt= udp_fd->uf_udpopt;
		udp_opt->nwuo_locaddr= ip_get_ifaddr(udp_fd->uf_port->
			up_ipdev);
...
size_t pack_size;
{
	...
	if (dst_addr == ip_get_ifaddr(udp_port->up_ipdev))
...
assert (udp_io_hdr);
	...
	/* The pseudo-header source must be the address ip_write() is about to
	 * stamp into ih_src, which is the interface's address as it stands NOW.
	 * Anything else -- a copy taken when this port started, say -- makes the
	 * checksum disagree with the header the datagram actually carries, and
	 * the far end drops every datagram this machine sends.
	 */
	locaddr= ip_get_ifaddr(udp_port->up_ipdev);
	chksum= pack_oneCsum(udp_hdr_pack);
	chksum= oneC_sum(chksum, (u16_t *)&locaddr, sizeof(ipaddr_t));
