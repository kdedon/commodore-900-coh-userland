...
/*
 * COHERENT libc errno is POSITIVE.  net/include/errno.h deliberately supplies
 * the Minix _SIGN convention (NEGATIVE) because the protocol core tests for
 * < 0, and the osdep layer translates at the boundary -- but this file is not
 * protocol core: it drives the libc directly (open/stat/unlink/mknod/link), so
 * its errno tests must use the COHERENT value.  Comparing a libc errno against
 * the negative constant is always false, which made a missing /dev/eth0 fatal
 * instead of ignorable: `init: /dev/eth0: Error 2' and the daemon exited before
 * creating any of its device nodes.  Negating the Minix constant keeps the two
 * in step without restating the number.
 */
#define COH_ENOENT	(-ENOENT)

static u8_t iftype[IP_PORT_MAX];	/* Interface in use as? */
static int ifdefault= -1;		/* Default network interface. */

static void fatal(label)
char *label;
{
...
/*
 * Make the daemon's own device nodes match inet.conf, silently.  Each of these
 * narrated its successful work upstream -- `rm', `mknod', `ln' per node, a
 * dozen lines every boot -- which says only that a routine step took the branch
 * it takes on every machine.  Every failure still goes through fatal(), which
 * names the node and the errno and stops.
 */
static void check_rm(device)
char *device;
{
	if (unlink(device) < 0) {
		if (errno == COH_ENOENT) return;
		fatal(device);
	}
}

static void check_mknod(device, mode, minor)
char *device;
mode_t mode;
int minor;
{
	...
	if (stat(device, &st) < 0) {
		if (errno != COH_ENOENT) fatal(device);
	} else {
...
static void check_dev(type, ifno)
int type;
int ifno;
{
...
static unsigned number(str, max)
char *str;
unsigned max;
{
...
void read_conf()
{
	...
	/*
	 * The configuration is fully parsed here and nothing reads cfg_fd
	 * again, so the descriptor goes back.  Held for the life of the daemon
	 * it was not merely untidy: a descriptor is what a channel is made of,
	 * and this one -- alongside stdin, stdout, stderr and the rendezvous
	 * FIFO -- was the difference between fifteen usable descriptors and
	 * sixteen, which is one connection for the whole machine.
	 */
	(void) close(cfg_fd);
	cfg_fd= -1;
