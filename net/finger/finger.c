...
/*
 * A COHERENT utmp record is (ut_line, ut_name, ut_time) and a lastlog record is
 * the same shape: neither says where a login came from, so the host a user
 * arrived from is not knowable here and the host column is always empty.  The
 * size is the field's own, since nothing else fixes it.
 */
#define HMAX 16
...
static void decode();
static void fwclose();
static int netfinger ();
static int matchcmp ();
static void quickprint ();
static void shortprint ();
static void personprint ();
static int AlreadyPrinted();
static int AnyMail ();
static struct passwd *pwdcopy();
static void findidle ();
static int ltimeprint ();
static void stimeprint ();
static void findwhen ();
static int namecmp ();
