...
#include "ansi.h"
#include "extern.h"
#if !defined(sun) && !defined(COHERENT) /* we want to know about TIOCGWINSZ. jw. */
# include <sys/ioctl.h>
#endif
...
/*
 * the termcap routines need this to work.  ospeed is libterm's own
 * (base/lib/libterm/tputs.c defines it and reads it for the padding count);
 * BC and UP are the caller's, which tgoto() declares extern.
 */
extern short ospeed;
char *BC;
char *UP;
char PC;			/* libterm's pad character; NUL by default */
...
InitTermcap()
{
  ...
  if ((s = e_tgetstr("pc", &tp)) != NULL)
    PC = *s;			/* tputs pads with this, not necessarily NUL */
