 ...
 *
 ****************************************************************
 * $Id: screen.h,v 1.4 92/02/04 21:38:19 jnweiger Exp $ FAU
 */
...
#if defined(M_XENIX) || defined(COHERENT)
typedef int pid_t;
typedef int gid_t;
typedef int uid_t;
#endif
...
#else
#define Free(a) {if ((a) == 0) abort(); else free((char *)(a)); (a)=0;}
#endif
...
/*
 * Scrollback.  Two independent limits bite on this machine and both are much
 * lower than screen's own 3000:
 *
 *  - int is SIXTEEN bits.  mark.c addresses a selection linearly as
 *    y * screenwidth + x (ten places, e.g. MarkRewrite), so the total row count
 *    histheight + screenheight must satisfy rows * screenwidth < 32767: 409 rows
 *    at 80 columns, 248 at 132.  200 + a 24-line display is inside both.
 *
 *  - each scrollback line is three malloc'd row buffers (image, attribute,
 *    font).  With malloc's 8-byte header a row costs 3 * 88 = 264 bytes, so
 *    screen's default of 100 lines is 27 KB per window -- 110 KB over the four
 *    windows the pty driver has channels for, several 64K data segments' worth.
 *    One screenful is the default here; `screen -h N' or a `scrollback N' line
 *    in .screenrc buys more at 264 bytes a line.
 */
#define MAXHISTHEIGHT 200
#define DEFAULTHISTHEIGHT 24
