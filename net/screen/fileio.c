...
char *ubuf;
{
  ...
  switch ((enum RCcases) mid)
      ...
      else
	{
	  ...
	  for (st=0; st<2; st++)
            salt[st] = 'A' + (int)((time((long *)0) >> 6*st) % 26);
...
/*
 * stripdev() -- the name of a tty without its /dev/ prefix.  Outside the utmp
 * block below on purpose: the socket name is built from it whether or not this
 * screen writes utmp.
 */
char *
stripdev(nam)
...
char *name;
{
  ...
  (void) time(&u.ut_time);
  slot = (lseek(utmpfappend, 0L, 2) + 1) / sizeof u;
