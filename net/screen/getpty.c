	...
	for (l = "pqrstuvwxyz"; '\0' != *l; ++l) {
		...
		for (d = "0123456789abcdef"; *d; ++d) {
			...
			if ((f = open(PtyName, O_RDWR)) != -1) {
				...
				/*
				 * No more ptys on this letter.  ENOENT counts:
				 * COHERENT's /dev is a fixed list of nodes, so
				 * a missing ptyp4 means there is no ptyp5
				 * either -- without this the scan does all 176
