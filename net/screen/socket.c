...
int *fdp;
{
  ...
  found = *SockNamePtr;
  *SockNamePtr = '\0';
  if ((dirfd = open(SockPath, O_RDONLY)) < 0)
    {
      Msg(errno, "Cannot read %s", SockPath);
      /* NOTREACHED */
    }
  *SockNamePtr = found;
  found = 0;
  /*
   * Read the directory itself.  There is no opendir(3) in this libc and DIR is
   * `char *' in <sys/dir.h>, so walk the raw struct direct records; d_ino 0
   * marks a free slot and d_name is not terminated when it is DIRSIZ long.
   */
  dname[DIRSIZ] = '\0';
  while (read(dirfd, (char *) &dent, sizeof dent) == sizeof dent)
    {
      if (dent.d_ino == 0)
	continue;
      strncpy(dname, dent.d_name, DIRSIZ);
      Name = dname;
      /* 
...
#else
# if defined(COHERENT)
      if (mknod(SockPath, S_IFPIP | S_IEXEC | S_IWRITE | S_IREAD, 0))
# else
...
#if defined(SYSV) && !defined(COHERENT)
...
#else
/*VARARGS1*/
SendErrorMsg(fmt)		/* varargs through printf's %r; cf. Msg() */
char *fmt;
{
#endif
  register int s;
