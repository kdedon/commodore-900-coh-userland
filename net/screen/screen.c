...
#endif
/*
 * COHERENT keeps fd_set, the FD_* macros, FD_SETSIZE and struct timeval all in
 * <sys/select.h>, and its <sys/time.h> has no include guard, so taking both
 * redeclares struct timeval.  This header is REQUIRED, not decorative: without
 * it screen falls back to its own `struct fd_set { int fd_bits[1]; }' below,
 * which is TWO bytes where libc's select() reads and writes a four-byte long
 * mask -- so the call would scribble past the caller's fd_set and could not
 * name a descriptor above 15 at all (1 << 16 in a 16-bit int).
 */
#if defined(COHERENT)
...
#endif
#if !defined(sysV68) && !defined(M_XENIX) && !defined(COHERENT)
# include <sys/wait.h>
#endif
#include <sys/stat.h>
#if !defined(sgi) && !defined(COHERENT)
# include <sys/file.h>
#endif /* sgi */
#if !defined(sun) && !defined(COHERENT)
# include <sys/ioctl.h>
#endif /* sun */
...
static void CheckSocket __P((void));
#ifdef NOSIGCLD
static void CheckWindows __P((void));
#endif
...
#endif
static void GetHostName __P((char *, int));
 ...
 *
 * There is no uname(2) and no gethostname(3) in this libc: COHERENT 3.x has no
 * hostname anywhere in the kernel, so like the BSDs of its day it keeps one in a
 * file.  Same file and same fallback as net/netdb.c's gethostname(), which is
 * not linked here (it lives in libsocket, and screen wants no network).
 */
...
# ifdef TERMIO
  /*
   * tputs() indexes its delay table with ospeed, so leaving it 0 turns all
   * capability padding off.  With a termio there is no sg_ospeed: the rate is
   * the CBAUD field of c_cflag, whose codes are the same B0..B38400 ordinals
   * the table is built on.
   */
  ospeed = (short) (OldMode.tio.c_cflag & CBAUD);
# else
...
#endif
  /*
   * There is no getppid(2) on this system, so the backend cannot ask who its
   * attacher is after the fork -- take it here, where the parent IS the
   * attacher, and let the child inherit it.  cmd/talk and cmd/telnetd do the
   * same.
   */
  AttacherPid = getpid();
...
#ifdef NOSIGCLD
      /*
       * Always a bounded wait: the sweep below is the only thing that notices a
       * window whose process has gone, so the backend must not sleep for ever.
       */
      if (!status)
	tv.tv_sec = 1;
      tvp = &tv;
#else
      tvp = (status) ? &tv : (struct timeval *) 0;
#endif
      if ((nsel = select(FD_SETSIZE, &r, &w, &e, tvp)) < 0)
	{
...
#ifdef NOSIGCLD
      if (time(&nowcheck) != lastcheck)
	{
	  lastcheck = nowcheck;
	  CheckWindows();		/* may Finit() if the last window went */
	  CheckSocket();
	}
#endif
      if (InterruptPlease)
	  ...
	  if (			/* FD_ISSET(tmp, &w) && */
	      ...
	      if (slowpaste > 0)
		{
		  struct timeval t;
                  ...
                  debug1("slowpaste %d\n", slowpaste);
		  t.tv_usec = (long) slowpaste * 1000L;
		  t.tv_sec = 0;
...
/*
 * CheckSocket() -- our FIFO is still there, or make a new one.
 */
static void CheckSocket()
{
  ...
  if (stat(SockPath, &st) == -1)
    {
      debug1("CheckSocket: Yuck! cannot stat '%s'\n", SockPath);
      if (!RecoverSocket())
	{
    ...
    debug2("CheckSocket: stat '%s' o.k. (%03o)\n", SockPath, st.st_mode);
}

#ifdef NOSIGCLD
/*
 * CheckWindows() -- reap the windows whose process has gone.
 *
 * Nothing tells us: there is no SIGCLD, wait() has no WNOHANG, and this pty
 * driver reports a hangup by neither of the two routes a select()-driven server
 * would use -- ptypoll() never sets POLLHUP, and ptyread() answers EAGAIN for a
 * non-blocking master before it looks at the closed-slave state (p_mopen == 3),
 * which is the O_NDELAY that MakeWindow sets.
 *
 * What does change is the child itself.  ukill() skips a PSDEAD process, so
 * kill(pid, 0) on an unreaped child answers ESRCH -- and a wait() for a child
 * already dead returns immediately, which is what makes a blocking wait() safe
 * here.  Called about once a second from the main loop.
 */
static void CheckWindows()
{
  ...
  for (n = WinList; n != -1; n = p->WinLink)
	...
	continue;
      errno = 0;
      if (kill(p->wpid, 0) == -1 && errno == ESRCH)
	dead++;
    }
  errno = 0;
  while (dead-- > 0)
    DoWait();		/* one zombie per call, and each wait() returns at once */
}
#endif /* NOSIGCLD */
  ...
  while (GotSignal)
    {
      GotSignal = 0;
      DoWait();
#if defined(SYSV) && !defined(NOSIGCLD)
      signal(SIGCLD, SigChld);
#endif
    }
  CheckSocket();
...
/*
 * Claim a pty master.  getpty.c does the scan: it opens the MASTER only and
 * hands back both names.
 *
 * Every other OpenPTY() below also opens the slave and closes it again, as a
 * probe.  That must not happen here: a slave open is what raises this pty's
 * carrier (sys/drv/pty.c) and closing it lowers it, so the master would read
 * the EOF that the backend's read loop takes for a dead window.
 */
static int OpenPTY()
{
  return (getpseudotty(TtyName, PtyName));
}
...
#endif
#if defined(DEBUG) && !defined(NOSIGCLD)
# ifdef SYSV
...
#else
/*
 * There are no varargs here: no <stdarg.h>, no <varargs.h>, no vsprintf.  What
 * this library has instead is printf's own `%r' conversion, which takes the
 * ADDRESS of the first variable argument -- the format string -- and recurses
 * on the caller's frame from there.  So the extra parameters are not declared
 * at all, and each conversion consumes exactly the width its letter says.
 *
 * Declaring them (as `unsigned long p1..p6', which is what screen does on
 * 32-bit hosts) cannot work on this machine: an int argument is two bytes and a
 * char * is four, so reading six longs off the frame misreads the first
 * argument and every one after it.  cmd/mv.c's warn() is the same function in
 * the same shape.
 */
/*VARARGS2*/
Msg(err, fmt)
