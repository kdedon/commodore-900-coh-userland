...
#ifndef lint
static char rcsid[] = "$Id: unpack.c,v 1.1 1993/01/21 18:32:58 jloup Exp $";
#endif
...
{ \
  while (valid < (bits)) bitbuf = (bitbuf<<8) | get_byte(), valid += 8; \
...
local void read_tree()
{
    ...
    orig_len = 0;
    for (n = 1; n <= 4; n++) orig_len = (orig_len << 8) | get_byte();

    max_len = get_byte(); /* maximum bit length of Huffman codes */
