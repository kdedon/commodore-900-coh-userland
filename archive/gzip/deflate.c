...
#ifdef COHERENT
/* window[] is shared with the decompressor, which needs WSIZE bytes of
 * history.  The compressor's window is two halves of DEFL_WSIZE, so the
 * same 0x8000-byte array serves both, and prev[] and head[] then also fit
 * inside one 64K object.  See gzip.h.
 */
#  undef  WSIZE
#  define WSIZE DEFL_WSIZE
#  define HASH_BITS 14
#endif

#ifndef lint
static char rcsid[] = "$Id: deflate.c,v 0.8 1993/01/26 19:12:42 jloup Exp $";
#endif
