...
#ifndef	INBUFSIZ
#  define INBUFSIZ  16384  /* input buffer size */
#endif
...
extern uch inbuf[INBUFSIZ+64]; /* input buf (+64 for unlzw, to be removed) */
...
/* The deflate window is sized independently of the inflate window.  A
 * decoder must hold WSIZE bytes of history because the deflate format
 * permits match distances up to 32K, so WSIZE stays at 0x8000 and window[]
 * is 0x8000 bytes.  An encoder is free to emit shorter distances; deflate.c
 * therefore redefines WSIZE to DEFL_WSIZE and lays its two half-windows,
 * its hash chains and its hash heads out inside the 64K single-object
 * limit.  The cost is a slightly larger compressed file, not a format
 * incompatibility.
 */
#ifndef DEFL_WSIZE
#  define DEFL_WSIZE 0x4000
#endif
...
extern char *xmalloc      OF((unsigned int size));
