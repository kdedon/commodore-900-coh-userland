...
/* Function: Flush iop.
 * Same routine as in stdio, but aborts program on error.
 */
fflsbuf(c, iop)	
	unsigned c; 
	FILE  *iop;
...
/* The library fprintf handles the variable argument list, so RCS uses it
 * directly and checks for write errors through fflsbuf and fflush.
 */
